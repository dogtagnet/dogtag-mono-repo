...
async function whitelistedAtIssuanceCheck(
  ...
  try {
    history = await reader.grants(governing, clone, signer);
    reads.push({
  ...
  // An EMPTY history is a DEFINITE refusal and needs no vocabulary guard: there is one authority
  // vocabulary, so a log that records no grant is evidence about the credential rather than about
  // the query. A read that FAILED never reaches here - it returned `unavailable` above.

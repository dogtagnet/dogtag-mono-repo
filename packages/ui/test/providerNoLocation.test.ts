...
describe("a geo-less row is accepted", () => {
  it("carries geo as null - never an empty object, and never a 0,0 substitute", async () => {
    ...
    expect(provider.contact.phone).toBe("+65 6123 4567");
    expect(provider.contact.whatsapp).toBe("+65 9123 4567");
    expect(provider.contact.telegram).toBe("@pawshmobile");
    expect(provider.contact.email).toBe("hello@pawsh.test");
    expect(provider.contact.website).toBe("https://pawsh.test");
  });
  ...
  it("fills every contact channel with null when the server sends no contact key at all", async () => {
    ...
    expect(result.state).toBe("found");
    if (result.state !== "found") return;
    expect(result.providers[0].contact).toEqual({
      phone: null,
      whatsapp: null,
      telegram: null,
      email: null,
  ...
  it("reads a blank contact string as the absent channel it means", async () => {
    ...
    expect(result.state).toBe("found");
    if (result.state !== "found") return;
    expect(result.providers[0].contact.phone).toBeNull();
    expect(result.providers[0].contact.email).toBeNull();
    expect(result.providers[0].contact.website).toBe("https://pawsh.test");
  });
});
...
describe("a location-less provider is listed and contactable, but never placed", () => {
  ...
  it("names a provider with neither a location nor a channel, and still lists it", () => {
    const empty: DirectoryProvider = {
      ...contactOnly,
      providerId: "p-empty",
      contact: { phone: null, whatsapp: null, telegram: null, email: null, website: null },
    };

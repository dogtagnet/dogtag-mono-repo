...
contract RedeployVerificationRegistry is Script {
    ...
    function run() external {
        ...
        console2.log(
            "  canonical: contracts/deployments/roax.json (keep old as VerificationRegistry_4arg_legacy)"
        );
        console2.log(
            "  mobile bundles (compile-time; rebuild+reinstall): apps/ios/DogTag/roax.json, apps/android/app/src/main/assets/roax.json"
        );
        console2.log(
            "  web/shared config: packages/ui/src/wallet/contracts.ts, stacks/owner/web/src/lib/config.ts"
        );
        console2.log(
            "  indexer default (anti-spoof gate): stacks/indexer/api/src/main.rs DEFAULT_VREG + stacks/indexer/.env.example"
        );
        ...
        console2.log(
            "  docs: README.md + AGENTS.md + docs/{DEPLOY,DEPLOYMENT,DEMO,GROOMER_ZK_DEMO,REMOTE_DEPLOYMENT,CEREMONY_RUNBOOK}.md"
        );

...
/// @title ProtocolVersions — the two CURRENT protocol versions, as published to `ProtocolRegistry`.
...
library ProtocolVersions {
    ...
    /// @notice `dogtag-levela/1` — the pre-M7 Level-A verification trio (still live until cutover).
    function levelA() internal pure returns (ProtocolRegistry.Version memory) {
        return ProtocolRegistry.Version({
            versionId: keccak256(bytes(LEVEL_A_VERSION)),
            ...
            minAppVersion: "1.0.0",
            publishedAt: 0, // stamped by executeVersion
            active: false // set true by executeVersion
        });
    }

    /// @notice `dogtag-levelb/1` — the canonical M5 Level-B owner-blind consent trio.
    function levelB() internal pure returns (ProtocolRegistry.Version memory) {
        return ProtocolRegistry.Version({
            versionId: keccak256(bytes(LEVEL_B_VERSION)),
            factory: 0xd3179AbBfb0274D0a5F7017d76015A93C159511D,

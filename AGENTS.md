...
- **`registry-governs-issuer` is the only check that can void a GREEN row.** `IssuerRegistry._wl` is a per-contract mapping and `DogTagIssuer.onlyWhitelisted` consults the one registry in the clone's own `registry` slot, so a whitelist answer from a different instance is a confident answer to a different question - in either direction. It reads `DogTagIssuer.registry()`. For a clone resolved through a MATCHED factory/registry pair it can only pass (`registry` is written once from the factory's `immutable registry`, no setter), so a failure is diagnostic of THIS CLIENT'S configuration and the finding says so in those words.

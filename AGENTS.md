...
- **Three-role showcase**: `scripts/demo-up.sh` boots all role stacks as separate services (admin/vet/groomer/government + portals). `scripts/e2e-roles.sh` (default = hermetic government ISSUE→VERIFY on `GOV_CHAIN_BACKEND=mem`, no deps; `--live` = vet ISSUES → government VERIFIES → government ISSUES across the running stacks over ROAX, needs `contracts/.env`). `government-api tests/cross_role.rs` codifies "vet ISSUES → government VERIFIES" deterministically over MemChain. See `docs/ROLE_APPS.md` §8.
  ...
  **The propose-only DECLARATION (`ADMIN_PROPOSE_ONLY`, alias `ALLOW_UNAUTHORIZED_ADMIN_SIGNER`) is what makes a proposal legible.** `disposition:"proposed"` is BOTH the designed out-of-band-signing flow and what a retired key produces, so grant/revoke responses report a **tri-state `outcome`** rather than one boolean: `executed` (≥1 action broadcast), `proposed_by_design` (nothing broadcast **and** propose-only declared - a correct outcome, calm `warning`, never says the key is wrong), or `proposed_unauthorized` (nothing broadcast, not declared - the loud wrong-key `warning`). `executed` is retained for back-compat, `warning` text differs per outcome, and the ISSUER_ROLE action is folded into the determination, so "nothing was broadcast" can only be claimed when NOT ONE action landed across the whitelist actions AND `issuerRole`. It is only a REPORTING input - it never changes what is dispatched, and holdership is always read live from the chain. `demo-up.sh` passes it through (it is the same flag as its own boot override); the admin portal drives the toast off `outcome` (green / amber / red) and never re-derives the case from `actions`.

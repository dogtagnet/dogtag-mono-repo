...
- **Provider directory (`GET /v1/businesses`) — interim admin source, on-chain source after S-10.** `Directory` retains the last complete, successfully decoded public admin `/v1/businesses` response, including nullable `geo` and all five optional BUSINESS contacts (`phone`, `whatsapp`, `telegram`, `email`, `website`), and never exposes `hmacSecret`. Static signer-name seeds and issuer applications are NOT provider rows because they cannot supply those fields. Never-loaded/unconfigured is 503; a successfully read empty source is `200 {"businesses":[]}`. Bare GET returns the WHOLE set in source order. Optional `name` (trimmed case/diacritic-insensitive substring), `kind` (trimmed case-insensitive exact; `type` is its existing-client compatibility alias and both together are rejected), and the all-or-none `searchCenterLat` + `searchCenterLng` + `searchRadiusKm` group filter with AND semantics; `geo:null` remains in full/name/kind results and cannot match a radius. `near`, bare `lat`/`lng`, `radius`, current-GPS aliases, bbox, geohash, pagination, and unknown params are rejected rather than ignored.

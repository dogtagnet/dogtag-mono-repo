...
- **Provider directory (`GET /v1/businesses`) — interim admin source, on-chain source after S-10.** `Directory` retains the last complete, successfully decoded public admin `/v1/businesses` response, including nullable `geo` and all five optional BUSINESS contacts (`phone`, `whatsapp`, `telegram`, `email`, `website`), and never exposes `hmacSecret`. Static signer-name seeds and issuer applications are NOT provider rows because they cannot supply those fields. Never-loaded/unconfigured is 503; a successfully read empty source is `200 {"businesses":[]}`. Bare GET returns the WHOLE set in source order, with every published kind and no hidden default. Optional `name` (trimmed case/diacritic-insensitive substring), repeatable `kind` (trimmed case-insensitive exact; repeated values OR, e.g. `kind=vet&kind=groomer`), and the all-or-none `searchCenterLat` + `searchCenterLng` + `searchRadiusKm` group filter with AND semantics; `type` is the same repeatable existing-client compatibility alias, but mixing the two spellings is rejected. Kinds are opaque caller-selected values, not a server allowlist: current rows may be vet/groomer/admin/government, and future values still filter normally. `geo:null` remains in full/name/kind results and cannot match a radius. `near`, bare `lat`/`lng`, `radius`, current-GPS aliases, bbox, geohash, pagination, partial place queries, and unknown params are rejected rather than ignored.
  ...
  - **Search-center privacy boundary is load-bearing.** A search center is a place the user deliberately typed, searched for, or picked on a map, so sending it is disclosed intent like sending a provider name. The live/current GPS fix is involuntary and continuous: native `ProviderDirectory.read()` remains no-argument, Nearby fetches the bare full set, and current-position distance/sort remains on-device. The server sees only coordinate numbers and cannot prove provenance; never feed live GPS into `searchCenter*`. Any future server-side current-position feature needs a separate, explicit, disclosed caller seam. The route's strict names plus Android/iOS exact bare-URL tests guard this boundary.
    Place autocomplete/geocoding and list-vs-map presentation are app concerns: this endpoint accepts only the resolved chosen center and never forwards a partial place query. Tapping a provider may hand its destination to the OS maps app.
  - **Full-fetch scale:** gzip is installed by the library router, not only the standalone binary. At the planning estimate of ~100 compressed bytes/provider, 50,000 providers produce ~5,000,000 bytes (4.77 MiB; 52,429 at 5 MiB), the point where a cold cellular full fetch becomes a live question. Without compression the 200–400 byte raw estimate reaches 5 MB at 25,000–12,500 rows. Delta sync is the next full-set optimization; the deliberate search filters already exist.
...
## Distance is computed on the device; the position never becomes a request parameter (`packages/ui/src/geo/`)
...
- Current-position permission is when-in-use and is requested only after the holder taps **Use my
  location**. Nearby's alternative origin remains manually entered decimal latitude/longitude and is
  parsed on-device. A separate featureful directory search may deliberately disclose a place the user
  typed, searched for, or picked on a map through `searchCenter*`; it must never silently substitute
  the live GPS fix. Remote geocoding/place search is therefore allowed only as that explicit user act,
  with its chosen-location disclosure made clear.

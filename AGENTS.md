...
The dogtag-governed discovery TRUST ANCHOR (M7 §5.1, lock B): a small read-mostly contract mapping a protocol version to its trio + verifier + artifact fetch-pins, so an app can validate a platform's version CLAIM against a dogtag-owned root of truth. **Additive** - it references the existing trio/verifier addresses but deploys/changes nothing else; NO circuit/VK/ceremony change. The resolve-GET extension + app-side validation are **P4** (not here); federation contracts are **P5**.

- **The contract** (`contracts/src/ProtocolRegistry.sol`, `AccessControlDefaultAdminRules`, `PUBLISHER_ROLE`): the `Version` struct is EXACTLY §5.1 - `versionId` (== `keccak256(bytes("dogtag-levelb/1"))`, the map key), `factory`/`verificationRegistry`/`sbt` (trio), `verifier`, `circuitId`, the four `*Sha256` fetch-pins, `artifactBaseUrl`, `minAppVersion`, `publishedAt`, `active`. `versions` mapping + enumerable `versionList`.
...
- **Timelocked publish, immediate deprecate.** Publish is `proposeVersion(Version)` → (2-day `PUBLISH_TIMELOCK`) → `executeVersion(bytes32 id)`, MIRRORING `VerificationRegistryConsent.proposeZkVerifier`/`executeZkVerifier` (a fresh propose resets the ETA; execute stamps `publishedAt=block.timestamp`+`active=true`, appends to `versionList` only on FIRST publish so a swap-republish never dups). `deprecateVersion` flips `active=false` immediately (a safety lever), NEVER deletes - history stays pinned so old records self-route (§7.3). `getVersion` fails closed on an unknown id.
...
- **The version-coherence check is a CALLER-INTEGRITY guard, NOT the downgrade defense.** The caller resolves the anchor BY `claims.protocolVersion`, so a platform that merely CLAIMS an older version resolves that version's own legitimate record and coheres with it; the check only catches "the caller resolved the wrong anchor". The app is version-AGNOSTIC **by design** (lock C: nothing bundled - it discovers the version), so there is deliberately NO `expected_version` to pin against - adding one would contradict the architecture. The version-DOWNGRADE defense is therefore **OPERATIONAL**: dogtag MUST `deprecateVersion` superseded versions in the `ProtocolRegistry` (once `dogtag-levelb/1` is the standard, `dogtag-levela/1` MUST be marked `active=false`) so the validator's `active` check rejects them, backed by `minAppVersion` enforcement. Publishing a superseded version as still-`active` silently permits a downgrade onto it.
That operational lever is now **WIRED, not merely documented**: `active` is a real member of `dogtag_prover::manifest::OnchainVersion` (mirroring the Solidity `ProtocolRegistry.Version.active`) and rides through `reconcile` into `Reconciliation::authoritative`, from which `anchor_from_reconciliation` SOURCES it - so a chain-deprecated version yields `TrustedAnchor { active: false }` and the validator fails closed with `DeprecatedVersion`.
...
P4 still does NOT add a live Rust `ProtocolRegistry` eth-call reader (P3 deferred that) - the app reads `getVersion` natively into `TrustedAnchor.active` - but the bit now flows end-to-end through the reconcile/mapping path, so the defense is enforceable from both the app path and the server path.
...
- **Server-side trust-tier mapping** (`stacks/vet/api/src/discovery.rs`): `anchor_from_manifest(&Manifest, active)` / `anchor_from_reconciliation(&Reconciliation)` map the prover-crate manifest types into `TrustedAnchor`. Note the asymmetry: the ONLINE reconcile path takes NO `active` param (it sources the bit from the on-chain record - see the downgrade-defense note above), while the OFFLINE manifest-only path still takes one and its callers pass `true`, since a served manifest carries no lifecycle bit and is presumed active. The manifest is the source of the READABLE fields (on-chain `getVersion` returns `circuitId` as a keccak hash and carries no version string / no chain id); on-chain precedence is enforced by P3 `reconcile` BEFORE a manifest may feed the validator (`anchor_from_reconciliation` returns the conflicts if the manifest disagrees). This mapping lives in vet-api (which links both crates), keeping standard prover-free. The signed-manifest-fallback path is covered by `stacks/vet/api/tests/discovery_validation.rs`.

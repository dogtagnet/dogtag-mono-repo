...
- **Deployed ROAX `Groth16VerifierConsent`:** _PENDING_ — needs `contracts/.env` (`ROAX_RPC` + `DEPLOYER_PRIVATE_KEY`, the `0x119F8c…` deployer EOA); recorded in `contracts/deployments/roax.json` on deploy. Wiring it into `VerificationRegistry` is **M4** (NOT done here).

...
**`MemChain` on both backends models this with a synthetic log clock**, so CALL ORDER IS LOG ORDER: a test writes "delisted after issuance" by delisting after it issued. "Delisted BEFORE" cannot be driven — issuance is itself gated on the whitelist, so the backend preflight refuses it — and is seeded with `set_grant_history`, for the same reason `with_hostile_clone` exists. Government's `MemChain` also needs `with_registry` when NOTHING is ever whitelisted: a real clone answers `registry()` regardless, so a fake that only learns the address from the first grant reports UNDETERMINED where the chain would report a definite refusal.

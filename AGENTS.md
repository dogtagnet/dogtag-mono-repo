...
- **Timelocked publish, immediate deprecate - on BOTH axes and on the binding.** Each write is `propose…` → (2-day `PUBLISH_TIMELOCK`) → `execute…`, MIRRORING `VerificationRegistryConsent.proposeZkVerifier`/`executeZkVerifier` (a fresh propose resets the ETA; execute stamps `publishedAt=block.timestamp`+`active=true` and appends to the list only on FIRST publish so a swap-republish never dups): `proposeContractSet`/`executeContractSet`, `proposeArtifactSet`/`executeArtifactSet`, and `proposeArtifactBinding`/`executeArtifactBinding`. The binding is timelocked because it is the pointer an app follows to decide which bytes to fetch - a one-transaction repoint is exactly the attack the window exists to catch.
...
- **Publish scripts** (`DeployProtocolRegistry.s.sol` + `PublishProtocolVersions.s.sol`): deploy is admin/publisher = governance `0x8E27E117…`; publish is TWO phases (`…Propose`, then `…Execute` after the timelock), reading the registry address from `PROTOCOL_REGISTRY`. `ProtocolVersions.sol` now authors each level as THREE values - `levelBContracts()`, `levelBArtifacts()`, and the binding - so the FIRST publication uses the two-axis scheme from the start (there is nothing to migrate). Both `dogtag-levela/1` + `dogtag-levelb/1` publish with the confirmed roax.json trio addresses + the frozen pins. The registry is NOT yet on-chain (no address in `deployments/roax.json` yet - deploy + record it at go-live).
...
## App anchor resolver + version-aware Level-B selector (M-4 PR3, iOS + Android)

The MOBILE consumer of the P4 machinery above: the app-side realization of the P4 promise that "the app reads `getContractSet`/`getActiveArtifactSet` natively into the two separate `TrustedAnchor` bits". Before an owner-hidden (`mode == "levelb"`) verify session is acted on, the phone RESOLVES the dogtag `ProtocolRegistry` anchor for `dogtag-levelb/1` and runs the shipped `validateDiscovery` binding, fail-closed on every axis - the anti-redirect gate that stops a lying platform steering an owner-hidden proof onto an attacker registry/chain. It adds NO server/circuit/VK/ceremony change; it wires the already-shipped `validateDiscovery` (M7 P4) into `ScanScreen`. Mirrored file-for-file across `apps/ios/DogTag/*` and `apps/android/.../{net,ui/screens}/*`. Design/flow live in the M7 P4 section above + impl §3.10d; deploy/publish in `docs/PROTOCOL_REGISTRY_RUNBOOK.md`. This section owns the mobile sharp edges.

- **The `mode == "levelb"` gate is EXPLICIT, deliberately NOT `isZk` - the load-bearing seam (E-1).** `AnchorResolver.isLevelB(mode)` (a pure, case-insensitive `== "levelb"` string check) gates the WHOLE Level-B branch in `ScanScreen` - every `ProtocolRegistry` eth_call, the anchor build, and the `validateDiscovery` call. Gating on the existing `isZk` flag would be the bug: `isZk` is true for ANY non-normal/ecdsa mode, so it would drop a `levelb` session into the Level-A prover - exactly the silent class this milestone exists to prevent. Consequence (the acceptance bar): a non-`levelb` session makes ZERO `ProtocolRegistry` calls and the Level-A / ECDSA path is byte-for-byte untouched. Pinned by the `isLevelB` gate unit test.
- **`AnchorResolver.{swift,kt}` is kept PURE (Foundation-only / no Android framework, no FFI, no chain I/O)** so it compiles into the host-less unit-test targets (same discipline as `ZkeyAsset`/`QrPayload` - see "iOS unit tests"). It does the two jobs `validateDiscovery` leaves to the caller (its own doc: "RESOLVING the anchor is the CALLER's job"): the `isLevelB` gate, and DECODING the two `ProtocolRegistry` getter returns into plain records (`ContractSetRecord`/`ArtifactSetRecord`). The FFI glue that builds a `TrustedAnchor` and calls `validateDiscovery` lives in `ScanScreen` (which imports the FFI module; `AnchorResolver` deliberately does not). Tests: `apps/ios/DogTagTests/AnchorResolverTests.swift` + `apps/android/app/src/test/.../net/AnchorResolverTest.kt` (mode gate, both decoders, fail-closed on short/truncated returns). The iOS `project.yml` (+ regenerated `pbxproj`) adds `AnchorResolver.swift` + `AnchorResolverTests.swift` to the host-less `DogTagTests` target - keep the `sources:` list in step, and confirm no bundle resource was stripped from the `pbxproj` (the xcodegen trap).
- **The two decoders - `getContractSet` is a STATIC 8-word tuple, `getActiveArtifactSet` is DYNAMIC.** `decodeContractSet` reads the 8 inline words `[contractSetId, factory, verificationRegistry, sbt, verifier, circuitId, publishedAt, active]` and keeps only `contractSetId`(0)/`verificationRegistry`(2)/`circuitId`(5)/`active`(7). `decodeArtifactSet` follows the leading offset to the tuple head, reads `artifactSetId`(head 0) + `active`(head 8) directly, and FOLLOWS the `minAppVersion` offset (head word 6, relative to the tuple start) to its `[length][bytes]` tail. `artifactBaseUrl` (head 5) is intentionally NOT decoded - the app BUNDLES the proving artifacts, it does not fetch them.
- **GOLDEN-VECTOR HAZARD the tests caught: follow the head-word offset, never assume a tail POSITION.** The live Solidity ABI encoder places `artifactBaseUrl` BEFORE `minAppVersion` in the string tail (offsets `0x120` then `0x180`) - the OPPOSITE order from the hand-built vector - so a decoder that assumed `minAppVersion` sat at a fixed tail position would have failed OPEN on the anti-downgrade `minAppVersion` gate. The decoder follows head-word 6's offset and reads `minAppVersion == "1.4.0"` correctly. This is load-bearing because `minAppVersion` feeds `validateDiscovery`'s `appVersion >= minAppVersion` check - the one anchor field with NO other cross-check. Pinned by the `decodeArtifactSetGolden` cases (real `getContractSet`/`getActiveArtifactSet` return bytes captured from a throwaway Foundry run over the `ProtocolVersions` levelb set - the Foundry file was removed, not committed) in both test suites. A malformed/short/bad-UTF-8 `minAppVersion` fails CLOSED (nil/null), never reads as "new enough".
- **`RoaxRpc.getContractSet`/`getActiveArtifactSet` + `versionId` - fail-closed on any null.** `versionId(version)` = `keccak256(utf8(version))` as the `ProtocolRegistry` map key (`contractSetId`); `AnchorResolver.LEVEL_B_VERSION` (`dogtag-levelb/1`) is the query key. The two selectors are DERIVED via `functionSelector("getContractSet(bytes32)")` / `functionSelector("getActiveArtifactSet(bytes32)")` (never literals - see the mobile-selector note near the top). A null return - registry blank/unconfigured/unreachable, version unpublished, OR no artifact binding (`getActiveArtifactSet` already follows the on-chain `activeArtifactSetOf` pointer, so no separate `activeArtifactSetOf` call) - fails the Level-B branch closed and never touches the Level-A path.
- **`resolveExportSession` now parses the `unverifiedClaims` block into `UnverifiedClaims`; before, only `mode` was read.** Additive: the block is absent on every pre-M-4 / non-levelb response, and the Level-A path never reads it. Named `UnverifiedClaims` DELIBERATELY, distinct from the FFI `ConvenienceClaims` record `validateDiscovery` consumes - this is the raw parse the caller maps INTO that FFI record (`ScanScreen` does the mapping).
- **`roax.json` + `RoaxConfig`/`RoaxConfig.swift` gain a `protocolRegistry`/`ProtocolRegistry` field, blank until deploy => fail closed.** A blank address makes `RoaxRpc.getContractSet` return null, so the Level-B branch fails closed while the Level-A default path - which never reads this field - is untouched. **GO-LIVE COUPLING:** the two mobile bundles (`apps/ios/DogTag/roax.json` + `apps/android/app/src/main/assets/roax.json`) each carry their OWN blank copy; when the registry is deployed the address must be filled into BOTH and the apps rebuilt/reinstalled (the phones read no backend env - `docs/PROTOCOL_REGISTRY_RUNBOOK.md` + `docs/PRODUCTION_DEPLOYMENT.md` §2.4), else mobile Level-B stays fail-closed forever. This rides with the PR4 app rebuild.
- **`contractSetActive`/`artifactSetActive` are sourced SEPARATELY from the two records, never AND-ed** (the FFI `TrustedAnchor` doc requires both true independently; AND-ing would silently discard one axis's kill switch - see the P4 "Wire BOTH" note). `appVersion` is read from `CFBundleShortVersionString` (iOS) / `BuildConfig.VERSION_NAME` (Android) - neither was read before. `expectedPurpose` is the session's own `purpose` (D1 ruling): the app has no purpose independent of the scanned QR today, so `validateDiscovery`'s purpose check is INTENTIONALLY WEAK here (claim vs the same session it came from) - not a regression (it matches the Level-A path's posture), and the anti-redirect weight sits in the registry/chainId/version/versionId/both-active/`minAppVersion` checks, which all still fire. An independent app-side purpose is queued follow-up.
- **PR3 SCOPE = resolve -> validate -> fail-closed ONLY; on validate-success it STOPS at a "validated" status and does NOT prove.** The owner-hidden prover (`proveConsent`), the `/v1/verify/consent/levelb` submit, the nullifier-index switch to Level-B, and the app-version bump are PR4 - deliberately NOT here. Do NOT "finish" the Level-B branch by falling through to the Level-A prover. **D2 - ships SAFE inert-until-deploy:** the `ProtocolRegistry` is not yet on ROAX (the mobile `ProtocolRegistry` field is blank), so `getContractSet` returns null and the branch fails closed; and even once deployed, until PR4 bumps the app past the published `minAppVersion` (`1.4.0` floor vs the apps' `0.1`) `validateDiscovery` throws `AppTooOld`. Either way the Level-A default path is unaffected.

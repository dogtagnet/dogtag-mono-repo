...
- **Publish scripts** (`PublishProtocolVersions.s.sol`; the separate `DeployProtocolRegistry.s.sol` is gone - `Deploy.s.sol` stands the registry up with everything else): deploy is admin/publisher = governance `0x8E27E117…`; `PUBLISH_TIMELOCK_SECS` defaults to 2 days and `TESTNET_DEPLOY=true` is the mandatory loud opt-in for any non-default delay. Publish is TWO phases (`…Propose`, then `…Execute` at/after the printed ETAs), reading the registry address from `PROTOCOL_REGISTRY`. `ProtocolVersions.sol` authors the single owner-hidden `dogtag-levelb/1` version as THREE values - `levelBContracts()`, `levelBArtifacts()`, and the binding - so the FIRST publication uses the two-axis scheme from the start (there is nothing to migrate). **NOTHING IS PUBLISHED on the launch registry**: `getDiscoverySet(keccak256("dogtag-levelb/1"))` reverts there with the contract's own named reason `"unknown discovery set"` (verified 2026-08-03), a deliberate fail-closed answer, and the ledger's `_publication` note says the same. The getter is `getDiscoverySet` - a call to a selector the contract lacks reverts with EMPTY returndata, and reading that as "nothing is published" would be a dispatcher refusal mistaken for an answer. So an app validating a version claim against this anchor resolves nothing and fails closed - correct for an unpublished deployment, and the reason publishing is a deliberate captain-authorized step rather than part of the deploy. See `docs/PROTOCOL_REGISTRY_RUNBOOK.md`.

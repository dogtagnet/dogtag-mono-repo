...
- **Implemented in five places**: `packages/ui/src/wallet/verifyCredential.ts` (`verdict = integrity && onchain && issuerWhitelisted === true`), `stacks/government/api/src/routes.rs::verify` and `stacks/vet/api/src/routes.rs::verify_credential` (both `issuer_whitelisted == Some(true)`), `crates/dogtag-standard-rs/src/verify.rs::verify` (`credential_valid` ANDs `IssuerWhitelistState::permits_pass()`), and the mobile importers (`RoaxRpc.issuerWhitelistPillar` + `RecordImporter.foldIssuerWhitelist`, a MONOTONE fold applied *after* the issuance mapping so it can only tighten a verdict). An operator-supplied signer is only an optional *expected*-signer assertion that tightens the pillar further; it can never enable it.

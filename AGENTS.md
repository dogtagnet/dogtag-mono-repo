...
Deployed live on ROAX chain 135 on 2026-08-04 by one run of `contracts/script/Deploy.s.sol`, blocks
339337-339348, superseding the 2026-08-03 set in full. Addresses and their transaction hashes are in
`contracts/deployments/roax.json`, which is the only place they belong - do not transcribe them into
new prose.

**That redeploy was FORCED by two contracts and cascaded to nine, which is the shape to expect from
any change to `DogTagIssuer` or `ProviderRegistry`.** PRs #143 and #144 moved both runtimes (measured
against `cast code` on the previous instances, not inferred: `DogTagIssuer` 4022 -> 4739 bytes,
`ProviderRegistry` 20975 -> 21187). Every `immutable` that pins either one has to move with it -
`DogTagIssuerFactory` pins `implementation` AND `registry`, `VerificationRegistryConsent` pins
`providerRegistry` and `rootIndex` (the factory), `ProviderDirectory` pins `core`,
`ServiceDomainResolver` pins `core` and `factory`. Six forced; the SBT, the frozen verifier and the
discovery registry follow because `Deploy.s.sol` stands up a COMPLETE, INDEPENDENT set and cannot
partially replace a running one. Enumerate that graph with `grep -n immutable contracts/src/*.sol`
rather than from a list - a pin added later is a member of it.
...
- `cd contracts && forge test` - 307 tests over 13 suites (12 `.t.sol` files; one carries two contracts), measured 2026-08-04. **A fresh worktree has
...
- `cd apps/android && gradle test` - the JVM unit suites (this naming is a partial list, not the whole
  ...
  **That hard dependency is NOT by itself a reason to skip the suite, and treating it as one is how the Kotlin half of a mirrored change ships never having run.**
  Measured 2026-07-31 on an ALREADY-WARM Gradle and cargo tree: the cargo step took ~3-8s and a targeted `gradle :app:testDebugUnitTest --tests '*GrantInForceAtTest*'` took 7-18s end to end.
  A cold worktree additionally pays AGP dependency resolution, a full main+test Kotlin compile and the Robolectric fetch below, none of which was measured - so pay it once rather than trying to filter around it, since `dependsOn` is task-level and a `--tests` filter dodges none of it.
...
- **Mobile `eth_call` selectors must be DERIVED from the signature, never hard-coded.** `apps/*` hand-encode selectors in `RoaxRpc.kt` / `Net.swift` (no ABI lib). `isValid`'s was once the stale literal `0x6d04f0bc` (its comment *claimed* to be the keccak but wasn't) - that selector REVERTS on the deployed ROAX `DogTagIssuer` clone, so every mobile validity read silently fell through to `Unknown`/accept-with-caveat and a revoked credential never showed as revoked. The canonical selector is `keccak256("isValid(bytes32)")[:4] = 0x6a938567` (what viem, the alloy `sol!` ABI, vet-api `verify_credential`, and the web direct-RPC path in `packages/ui` all bind). It is now derived on-device via `Keccak256` (`RoaxRpc.functionSelector` / `Net.swift` `functionSelector`). **BOTH platforms derive EVERY selector and neither client holds a selector literal any more**: `RoaxRpc.kt` and `Net.swift` each derive the same twelve signatures (`isValid`, `isWhitelistedFor`, `consumed`, `profileRoot`, `isClone`, `domainOf`, `name()`, `issuedBy`, `rootIssuer`, `recordType()`, `getDiscoverySet`, `getActiveArtifactSet`; the retired `bindNonce`/`keyOf`/`ownerOf` reads went with the owner-revealing layer). **The discovery getter is `getDiscoverySet`, never `getContractSet`** - both clients derived the latter until it was corrected, and it is the sharpest name in the set to get wrong, because a selector the contract lacks reverts at the DISPATCHER with empty returndata, which decodes to the same nil as an unpublished version. **What is asymmetric is the PINNING, not the derivation.** Android pins ten of those twelve against `cast sig`-confirmed values (`RoaxRpcSelectorTest.kt` for the validity/whitelist/consumed/profileRoot set plus the issuer↔domain chain, `IssuerWhitelistPillarTest.kt` for `issuedBy`/`recordType()`); the two `ProtocolRegistry` getters are derived but unpinned. iOS pins none of its SELECTORS: `Net.swift` does compile into the host-less `DogTagTests` target (it is listed in `apps/ios/project.yml`), but `functionSelector` and every selector are `private static`, so no test in that bundle can call them - pinning them means widening that access, not adding a source file. That asymmetry is now NARROWED rather than closed: the grant-history LOG TOPICS and `registrySelector` the issuer-whitelist pillar added are deliberately internal, so `GrantAtIssuanceTests` pins them on both platforms. Topics were singled out because their failure mode is the worst in the set - a value derived at the wrong WIDTH matches no log at all, which reads exactly like "never granted". Verify any new mobile selector against the chain before shipping: `eth_call` a real clone on `https://devrpc.roax.net` - the correct selector returns a 32-byte word, a wrong one returns `execution reverted`. **There ARE clones to point that at now** - the 2026-08-04 verification walk left a VACCINATION and a TRAVEL_CLEARANCE clone live on the deployed core (addresses in the ledger's `_provisioning`), so the chain check is available and is the one to make. Where a selector still cannot be checked against a real contract, pin it against `cast sig` and say that is what you did, rather than reporting a chain check you could not make. Note mobile has only the single `isValid` bool (no `issuedAt`/`isRevoked` decomposition like web), so it renders revoked and never-anchored identically as "REVOKED / not anchored"; that is intentional, not a bug.
...
- **User-selectable RPC is chain transport only, and it is never a trust upgrade.** Android Profile
  ...
  **The `ProtocolRegistry` discovery-anchor pair is the ONE exception that does not honour the
  setting, and that exception is load-bearing.** `getDiscoverySet`/`getActiveArtifactSet` in the
...
- **OPS-0 (on-chain prereq) - SATISFIED for `TRAVEL_CLEARANCE` on the 2026-08-04 set, and the shape of the sequence changed again with #143.** The verification walk registered one provider and drove it to `canIssue == true` for a VACCINATION clone and a `TRAVEL_CLEARANCE` clone; addresses are in the ledger's `_provisioning`. `EU_HEALTH_CERT` still has no clone, so leave `EU_HEALTH_CERT_ISSUER_ADDR` unset - the API then reports that issuer as null and `/issue` dry-runs rather than anchoring somewhere wrong. **Neither `whitelistFor` NOR `setIssuanceCapability` exists any more**: #143 re-keyed the issuance grant onto the ADDRESS as a bitmask, so the grant is `setRights(account, RIGHT_ISSUE)` and carries no service and no record type. Onboarding is a registrar SEQUENCE on the core - `registerProvider` (which writes standing `PENDING`, so it alone leaves the provider inert) -> `setProviderStanding(ACTIVE)` -> `setServiceCreationApproval(providerId, recordType, true)`, then the provider deploys its own clone from the factory and the registrar completes it with `attachService` (which likewise writes service standing `PENDING`) -> `setServiceStanding(ACTIVE)` -> `setRights(signer, RIGHT_ISSUE)`, and finally the PROVIDER's own `repointService`. **Two traps, both measured on a live walk rather than reasoned about.** `confirmServiceOwner` is NOT a fresh-attach step and reverts `NoChange()` if you call one - `attachService` already wrote `confirmedOwner = liveOwner` and `ownerEpoch = 1`, and that function exists for a LATER owner handover. And `canIssue` stays **false** until `repointService` lands even with every other term held, while `canRevoke` and `isRecognizedIssuer` are already true - that is the nested ladder folding the current-service pointer, not a fault to debug. `scripts/demo-provision-government.sh` and `scripts/demo-up.sh` both still drive the old `whitelistFor` shape and have NOT been updated, so neither is a working path against this set. **The clone-must-be-bound-to-the-authority-the-stack-reads trap is unchanged in kind** and is why a preflight exists: a clone gated by its own `registry()` against a core nobody reads fails closed on the first issuance and passes every other check until then. Earlier government clones belong to a superseded factory, so `rootIssuer` resolves nothing for them here - they are not a target to reuse.
...
- **Then the signer, from that clone.** `DogTagIssuer.issuedBy(root)` is the H-1 originator (`issuedBy[r] = msg.sender`, set under `onlyWhitelisted`), so a genuinely issued root's signer was whitelisted for its record type at issuance by construction. Then the HISTORICAL grant question below — never `isWhitelistedFor`. The factory still comes from the verifier's config and the registry is derived from the clone it resolved; if either came from the document the attacker would supply both sides of the question and the pillar would be theatre.

#### The pillar asks a HISTORICAL question, and the authority is the clone's, not the config's

**The question is "was this signer authorised AT THE MOMENT this root was anchored", never "is it authorised now".** Delisting is forward-only — `DogTagIssuer.sol:82` says so in the contract's own source and `adminRevoke` exists as the retroactive lever precisely because `delistFor` is not one. The pillar used to read `isWhitelistedFor`, a CURRENT-state getter, so an ordinary key rotation, a retirement or a lapsed licence retroactively rendered every credential that signer had ever issued a forgery, fleet-wide. Do not "simplify" it back: `isWhitelistedFor` is still the right read for a FORWARD-looking question (may this relayer verify this purpose now?), which is what the consent preflights ask, and both backends keep it for exactly that.

The answer is reconstructed from LOGS, so any verifier with an RPC reproduces it without trusting this code: locate the clone's `RootIssued(root)`, read the governing registry's `Whitelisted`/`Delisted(recordType, signer)` history, and take the LAST event at or before the anchoring point. `issuedAt` is a unix TIMESTAMP and cannot be compared against a log height without a timestamp->block search, which is why the anchoring comes from the event. Ordering is `(blockNumber, logIndex)`; `logIndex` is block-scoped and therefore comparable ACROSS contracts within one block, which is the only reason a grant and an issuance landing in the same block can be sequenced at all.

**WHICH registry answers is a correctness question.** `IssuerRegistry._wl` and its events are PER CONTRACT, so the authority is the one `DogTagIssuer.registry()` names — never the verifier's separately-configured `ISSUER_REGISTRY_ADDR`. A merely mis-paired client would otherwise find no grant and print a definite refusal of a genuine credential: our own misconfiguration rendered as an accusation, the fail-closed mirror of the fail-open bug this pillar exists to prevent. It opens no trust surface, and is strictly better than the config: `registry` is written once in `initialize` from the factory's own `immutable registry`, has no setter, and the clone came from the factory THIS verifier configured — so for a factory-resolved clone the address is as anchored as the clone itself, and can no longer be mis-paired at all. The SDK's `RpcAdapter::whitelisted_at_issuance` therefore takes NO registry address, for the same reason `root_issuer` takes no factory address.

**Three outcomes, and the third is not a neighbour of the other two.** A definite `Authorized` passes; a governing registry that IS readable and whose log records no grant in force then is a definite `NotAuthorized` (an honest `issue()` cannot pass `onlyWhitelisted` in that state, so it is evidence about the credential); and an unreadable log, an unestablished authority or a missing anchoring event are UNDETERMINED — never a pass, never an accusation. An empty history is an ANSWER, not an absence of one; a read that FAILED never reaches the fold. Each surface then reports its inability in its own convention: vet-api answers 502 on a failed read, government-api the same, the SDK gates via `IssuerWhitelistState::Unresolved`, web leaves `issuerWhitelisted` null, and mobile degrades to `UNVERIFIED` with the reason.

**One fold rule, four languages, mirrored by hand.** `dogtag_standard::verify::grant_in_force_at` (Rust, shared by the SDK and both backends' chain clients), `grantInForceAt` in `packages/ui/src/wallet/contracts.ts` (shared by the verifier and the bench), `RoaxRpc.grantInForceAt` (Kotlin) and `RoaxRpc.grantInForceAt` in `apps/ios/DogTag/Net.swift`. Move them together; each mobile one has a mirrored test (`GrantInForceAtTest`, `GrantAtIssuanceTests`).

**A LOG CARRYING NO POSITION IS UNDETERMINED ON ALL FOUR, and it may be neither placed nor dropped.** A node can return a log with no `(blockNumber, logIndex)` — viem's shape for one it considers pending, `None` in alloy — and there are exactly two ways to answer anyway, both wrong in opposite directions: placing it at `(0, 0)` puts a grant at the very start of the chain, which sorts before every anchoring and can turn a delisted-before into an AUTHORISED, while dropping it removes the event that might have been the delisting and does the same. Rust `log_point`, Kotlin `RoaxRpc.logPoint` and Swift `RoaxRpc.logPoint` all answer undetermined; `packages/ui` reached the mandatory verdict once with `l.blockNumber ?? 0n`, and now returns the `UNPOSITIONED_LOG` symbol from BOTH `whitelistGrantHistory` and `rootIssuedAtLog` so `issuerWhitelisted` lands on `null` like its three siblings. A **symbol**, not a string: `"unpositioned".length` is truthy, so a string sentinel would read as a non-empty history to any `if (history.length)` consumer — the very could-not-check-as-an-answer shape, reintroduced inside the fix. The three outcomes stay apart everywhere — an EMPTY history is a definite refusal, a read that THREW is its own case, and this is a third. Note the web is deliberately the STRICTEST on the anchoring read: the siblings `filter_map`/`mapNotNull`/`compactMap` unpositioned `RootIssued` logs and take the min of the rest, which with a positioned sibling would move the anchoring FORWARD past a delisting; write-once `issuedAt` makes that state unreachable on an honest clone, so for every state a real chain produces the four agree exactly.

**A LOG TOPIC is the full 32-byte keccak, not a 4-byte selector, and confusing them fails SILENTLY** — the shorter value matches no log at all, which is indistinguishable from "this pair was never granted" and would refuse every genuine credential. Both hand-rolled clients therefore derive topics through a SEPARATE `eventTopic` helper and pin the results (`RoaxRpcSelectorTest`, `GrantAtIssuanceTests`); Rust and TS get theirs from `sol!`/viem. Canonical values, confirmed with `cast keccak`: `RootIssued(bytes32,address,uint256)` = `0xf8cd30a6…`, `Whitelisted(bytes32,address)` = `0x0ed68b47…`, `Delisted(bytes32,address)` = `0xf3af84db…`. Note this narrows the "iOS pins NONE of its derived values" asymmetry recorded above: `eventTopic`, `registrySelector` and the fold are deliberately internal (not `private static`) so the host-less `DogTagTests` can reach them, while the call selectors stay private and unpinned.

**Government's `at_block` bounds BOTH log reads as well as the `registry()` call**, so the pillar's answer is the same snapshot the block anchor beside the verdict claims; a grant landing mid-verification cannot change it. The web reader does the same via `toBlock`. Mobile pins nothing (it has no block-anchored verdict).

**VERIFIED LIVE on ROAX 2026-07-31, which the section below tells you to do before tightening this pillar.** The genuine government `TRAVEL_CLEARANCE` credential in `deployments/roax.json` passes all three new reads: `registry()` on clone `0xB5D6654d…` answers the configured `IssuerRegistry` `0xAEE54035…`; its `RootIssued` for root `0x02ae43c0…` sits at block 276218, logIndex 1; and that registry's log holds exactly one `Whitelisted(keccak("TRAVEL_CLEARANCE"), 0xc6c0904f…)` at block 276164, logIndex 0 — so the fold answers AUTHORIZED. The raw `eth_getLogs` in the exact shape the apps send (topic0 as a SET, `fromBlock: 0x0`, `toBlock: latest`) is accepted and filters correctly; note `cast logs` renders extra rows for the same query and is misleading here, so check with raw JSON-RPC.

**Cost, stated because it is on the MANDATORY path:** the pillar went from one `eth_call` to one call plus two `eth_getLogs`, and each goes through the chain guard's own `eth_chainId` probe — roughly 2 requests per credential to roughly 6, multiplied by list length on a mobile refresh. The generation probe adds one more, but ONLY when the grant history came back empty, so the hot path is unchanged and the extra call lands on the refusal path alone. The log reads are unbounded (`fromBlock: 0` → `latest`) in all four implementations; ROAX serves them, but a HOLDER-CHOSEN peer that caps log ranges will fail them. Every such failure is honest (mobile `UNVERIFIED` with the reason, backends 502, web rejects) rather than a false pass, so this is availability rather than correctness — but the indexer already chunks its scans (`stacks/indexer/api/src/chain.rs`), which is where to look if this ever needs bounding.

**The amplification, and why the obvious bound is refused.** Government's `POST /v1/verify` is **UNAUTHENTICATED by design**, so an anonymous and trivially looped caller now costs the operator TWO genesis-to-head log scans against the operator's own node per request, growing with chain length. State that cost accurately: both reads are **address- and topic-filtered** (the anchoring read to the resolved clone plus one `RootIssued` topic, the grant read to the governing registry plus the `Whitelisted`/`Delisted` pair), so it is not an unfiltered full scan. vet-api's `POST /verify/credential` has the same shape but is operator-gated, so its exposure is lower. **Do NOT close this with a `from_block` floor, and do not gate or rate-limit the route.** Any floor above the true deployment height silently hides an early `Whitelisted`, and a missing grant is not a soft failure — the fold reads it as a definite refusal of a genuine credential, which is exactly the fail-closed accusation this whole pillar exists to remove, so a wrong floor trades a correctness risk for a cost saving on the one surface that decides authenticity. The sanctioned close is a **configured floor anchored to the factory's own deployment block**, which no deployment carries today — a design, not a tweak. Recorded at the reads themselves in `stacks/government/api/src/chain.rs::whitelisted_at_issuance`.

**ONE branch deliberately keeps the current-state getter: vet-api's `expectedSignerState` on a FACTORY-LESS deployment.** With no clone resolved there is no trusted anchoring point or authority, so both could only come off the document's `documentStore` — letting a forged document neutralise what CLAUDE.md calls "the last check standing" there. It therefore still asks THIS deployment's own registry `isWhitelistedFor`. The cost is real and bounded: an operator who explicitly asserts a signer delisted AFTER it issued gets a definite refusal on a factory-less stack. It fails loudly, and configuring `FACTORY_ADDR` restores the anchor and with it the historical question. Closing it properly means reading the grant history from this deployment's registry while taking the anchoring point from the document's contract — a design, not a tweak.

**`MemChain` on both backends models this with a synthetic log clock**, so CALL ORDER IS LOG ORDER: a test writes "delisted after issuance" by delisting after it issued. "Delisted BEFORE" cannot be driven — issuance is itself gated on the whitelist, so the backend preflight refuses it — and is seeded with `set_grant_history`, for the same reason `with_hostile_clone` exists. **BOTH backends' `MemChain` now need `with_registry` when NOTHING is ever whitelisted** (vet gained it alongside government's): a real clone answers `registry()` regardless, so a fake that only learns the address from the first grant reports UNDETERMINED where the chain would report a definite refusal. That gap is no longer only a verify-path concern — the issuance preflight resolves its authority the same way, so a fixture with no grant at all makes an honest 403 arrive as a 502.

**THE PILLAR IS ITSELF A RECORD-TYPE CALLER — via LOGS, not a getter — and against generation 2 it produced a CONFIDENT FORGERY VERDICT.**
`Whitelisted(bytes32 indexed recordType, address indexed signer)` puts the record-type key in `topic1`, so the grant-history read fails against `ProviderRegistry` for exactly the reason the getter does — a record-type-keyed question asked of an authority that keys nothing by record type — only far more quietly, because nothing reverts and no answer looks wrong.
That contract records grants as `IssuanceCapabilitySet(service, signer, allowed)`: different name, different `topic0`, different argument shape. The filter matches NOTHING, and `grant_in_force_at(&[], _)`'s empty-history rule — deliberately a definite `NotAuthorized`, because on generation 1 an empty log really is evidence that `onlyWhitelisted` could not have passed — renders "we asked the wrong contract in the wrong language" as "this credential is forged". It reaches the unauthenticated `POST /v1/verify`.
**The guard: an empty history is a definite refusal ONLY when the authority positively speaks generation 1**, established by probing `isRecognizedIssuer` — the one selector `IssuerRegistry` provably does not implement (its entire external surface is `whitelistFor`/`delistFor`/`isWhitelistedFor`, and it has **no fallback**, so the call reverts there rather than answering).
**It is on ALL FIVE mirrored surfaces**, and only the two Rust backends at first: vet `chain.rs` and government `chain.rs` shipped it while `packages/ui/src/wallet/contracts.ts` (`authorityGenerationOf`, consumed by `verifyCredential.ts`), Kotlin `RoaxRpc.grantAtIssuance` and Swift `RoaxRpc.grantAtIssuance` still refused. The claim went out ahead of the code, which is its own defect — a global guarantee resting on two of five, with the gap due to fire at the C-9/C-10 client repoint rather than at review. The SDK path (`crates/dogtag-standard-rs/src/verify.rs`) needs no sixth change: its `ChainRpcAdapter` delegates to vet's `whitelisted_at_issuance` and inherits the guard.
Four things about that guard are load-bearing and each is easy to undo:
- **It is scoped to the EMPTY case**, because a NON-EMPTY history is itself proof the authority speaks generation 1 — those events came out of it. Only the empty case is ambiguous between "generation 1, never granted" and "generation 2, wrong vocabulary". So the probe fires on the refusal path alone and cannot perturb any answer #127 established. Making it unconditional silently stops refusing every never-granted signer; `an_empty_history_on_a_generation_one_registry_is_still_a_definite_refusal` catches that on both backends, and `aNonEmptyHistoryIsUnaffectedByWhateverTheProbeSaid` / `test_aNonEmptyHistoryIsUnaffectedByWhateverTheProbeSaid` pin the other half on mobile.
- **The probe's ANSWER is discarded.** `isRecognizedIssuer` is `_issuanceCapabilities[service][signer]` — current storage, no block, no root — so consuming its boolean would revert the pillar to a current-state getter under a new name, i.e. undo #127. It identifies the GENERATION and nothing else.
...
`scripts/verify-issuance-authority-mutations.sh` is the repeatable gate (15 mutations). It runs `cargo check` after applying each one and reports a **non-compiling mutation as INERT** rather than counting its red as evidence — this slice shipped exactly that mistake once, a mutation naming a method that did not exist, whose "red" was a build failure. Self-tested: deliberately breaking a mutation makes the harness say INERT and exit 1. It carries TWO runners, and the split is not cosmetic: `check` addresses an integration target (`--test <name>`) while `check_lib` addresses a unit test inside `src` (`--lib <name>`), which is the only place the alloy error classification can be pinned — pointing `check` at it would silently match no test and report a green.
...
`packages/ui/src/wallet/benchScenarios.ts` is the adversarial half: complete fraudulent records, each
with its own **scripted, network-free chain**, run through the REAL `runVerificationBench`, each
declaring WHICH check must refuse it.
`BENCH_SCENARIOS` is exported and the `/bench` page runs it in the browser.
It is distinct from `benchMutations.ts`, which edits a record the operator loaded; a scenario needs no
loaded record and can script chain states a live chain cannot be asked to produce.

- **A scenario asserts the FULL outcome vector, never one row.** "Something went red" is satisfied by a fixture that happens to be expired or a chain that could not be reached, so a suite built on it stays green while the check it is named after is dead. `BenchScenario.expected` is exhaustive over `BenchCheckId` and a test enforces that exhaustiveness, because a partial vector is exactly where "refused for the wrong reason" hides.
- **`genuineCredential` is a REQUIRED member of the catalogue.** A suite of nothing but frauds goes green against a verifier that refuses everything. There is also a test asserting each fraud differs from the control in a BOUNDED way rather than collapsing the whole report.
- **THE FINDING - FIXED, and the worked example of what the `knownDefect` machinery is FOR.** A genuine credential whose signer was delisted AFTER issuance was refused as a forgery, contradicting the standing ruling that delisting is FORWARD-ONLY. The bench pinned it: `expected` stated the protocol-correct answer, `knownDefect` pinned the wrong one the code produced, and a test asserted the two still DIFFERED - so when the verdict formula moved, the scenario went red and the pin had to be deleted rather than the finding quietly evaporating. That is exactly what happened; see "The issuer-whitelist pillar asks a HISTORICAL question" for the fix. `BENCH_SCENARIOS.filter(s => s.knownDefect)` is now EMPTY and a test asserts it, so the next finding gets the same treatment.
  The write-path argument that made the credential provably genuine is still worth keeping: `issue()` is `onlyWhitelisted`, sets `issuedBy[r] = msg.sender` in the same call, and `rootIssuer[r]` is write-once and writable only from inside a clone's `issue()` - so for any root resolvable through the factory, "this signer was authorised at issuance" is true BY CONSTRUCTION. It also bounded the blast radius in the safe direction while the defect stood: **a current-state whitelist read cannot produce a false PASS on a factory-resolved root**, only false negatives - which is why nothing had to be revoked when it was fixed.
- **NEVER adjust an expectation to match a bug - it is the one move that would make the catalogue worthless**, because it converts every future regression into a green tick. `BenchScenario.expected`/`expectedVerdict` state the PROTOCOL-CORRECT answer and stay correct where the code disagrees; `knownDefect` separately pins what the implementation really does today. The suite asserts BOTH **and that the two still differ**, so the day the defect is fixed the scenario goes red and whoever fixed it must delete `knownDefect` rather than the finding quietly evaporating. Both directions of the delisting pair are asserted as REQUIREMENTS (before -> must refuse, after -> must verify), never as observations.
- **`whitelisted-at-issuance` FOLDS THE HISTORY ITSELF, so it is a SECOND implementation of the empty-history rule and needs every guard the verifier's copy has.** The gating `issuer-whitelisted` row copies the verifier's verdict verbatim; this one does not - it re-reads the log through its own `GrantHistoryReader` seam and calls `grantInForceAt` here. So the generation guard had to be added TWICE, and adding it only to `verifyCredential.ts` left the forgery accusation intact one row down: gating row "unresolved", advisory row "recorded NO grant", which is the line a reader takes as the reason the credential is bad. The catalogue stayed green throughout, because no scenario scripted generation 2 - the case it had just gone silent on. Generalize it: a bench row that RE-DERIVES rather than reports is a second copy of the rule, and "the seam is injected" is not the same as "the rule is shared". Both guards are scoped on `history.length`, never on "no event at or before the anchoring" - a non-empty history whose grants all postdate the anchoring is unambiguous and must keep its definite refusal; `still refuses when the log is non-empty but every grant POSTDATES the anchoring` is the only case that catches that widening. The bench's probe also needs its OWN failure switch in the fakes (`benchAuthorityGeneration`, beside `benchIssuerRegistry` and for the same reason): one shared key cannot fail this row's probe without also failing the verifier's, which rejects the whole run, so the row comes back `could-not-run` for an unrelated reason and the test passes while pinning nothing.
- **`whitelisted-at-issuance` reads the registry's LOGS, not the getter** - `Whitelisted`/`Delisted` are indexed on exactly the `(recordType, signer)` pair, so the whole grant history is one filtered `eth_getLogs`. It sequences against the clone's own `RootIssued` log rather than against `issuedAt`, which is a unix TIMESTAMP and would need a timestamp->block search. Ordering is `(blockNumber, logIndex)`: `logIndex` is block-scoped and therefore comparable ACROSS contracts within one block, which is what sequences a grant and an issuance landing in the same block. No log read may produce a pass.
  **WHICH registry it asks is a correctness question, not a convenience one, and it shipped wrong once.** `IssuerRegistry._wl` and its events are PER CONTRACT, so the grant history comes from the GOVERNING registry - the address `DogTagIssuer.registry()` answered, sourced from the read `registry-governs-issuer` records one row earlier (which is why `runVerificationBench` runs the two in that order, and why the ordering comment there says so). Never from the client-configured `registryAddr`: a client merely pointed at the wrong instance would find no grant at all and the row would print a definite refusal of a genuine credential - our own misconfiguration rendered as an accusation, the fail-closed mirror of the fail-open bug this surface exists to prevent, and the exact thing the sibling row's own doc comment forbids. This opens no trust surface, because that address is read from the clone the FACTORY resolved, never from the document.
  **The three outcomes are NOT all `could-not-run`, and the difference is the point.** An unreadable log read, and a governing registry that cannot be established at all (the `registry()` read failed, answered zero, or was never made), are `could-not-run` - our inability to check may never be stated as a verdict about the subject. But a GOVERNING registry that IS readable and whose own log records no grant at or before the anchoring point is a definite **`fail`**: that is evidence about the credential, not about us. Do not "simplify" those three into one answer in either direction.
- **`registry-governs-issuer` VOIDS NOTHING beside it any more, and that changed when the pillar did.** It used to be the one check that could void a GREEN row: while the whitelist pillar read this client's CONFIGURED registry, a `true` from a foreign instance passed a signer the clone never authorised. Both whitelist rows now source their authority from `DogTagIssuer.registry()` off the factory-resolved clone (see the pillar's own section), so a mis-paired configured registry no longer reaches either answer. What the row reports now is a fault in THIS CLIENT - its `registryAddr`/`factoryAddr` pair names a registry that governs none of the clones its factory deploys - which is still worth telling an operator, because everything else aimed at `registryAddr` (a grant console, a whitelist viewer, an operator about to delist) is pointed at a contract that decides nothing here. It still reads `DogTagIssuer.registry()`, is still `gatesVerdict: false`, and `foreignRegistry` still expects it to FAIL while both whitelist rows PASS. For a clone resolved through a MATCHED pair it can only pass (`registry` is written once from the factory's `immutable registry`, no setter), so a failure remains diagnostic of configuration and the finding says so in those words.
  **Neither this row nor `whitelisted-at-issuance` may report an UNREACHABLE factory as one that answered with nothing.** Both derive their clone from the recorded `rootIssuer` read, and only a read that SUCCEEDED with the zero address licenses "none was resolved"; a read that threw goes through the shared three-way `unobservedReason` split and names the failure instead. Reachable on every run of `wrongChainEndpoint` - the one scenario whose whole point is naming the chain mismatch - so it is pinned in both suites. Do NOT close this by adding a zero-value branch to `unobservedReason` itself: `readBackedCheck` shares that helper, and there an OK `issuedAt` read legitimately has value `"0"`.
- **Both new rows are `gatesVerdict: false`, and that is the finding rather than a hedge.** The bench copies the verifier's formula rather than competing with it. The value is the ASYMMETRY being legible: `issuer-whitelisted: fail (gating)` beside `whitelisted-at-issuance: pass (advisory)` tells an operator the refusal is about the signer's status today, not about whether the credential is real. The page marks advisory chips with a dagger and prints every failing row's `finding` inline - a finding reachable only by hovering a chip is not reported.
...
- **Publish scripts** (`PublishProtocolVersions.s.sol`; the separate `DeployProtocolRegistry.s.sol` is gone - `Deploy.s.sol` stands the registry up with everything else): deploy is admin/publisher = governance `0x8E27E117…`; `PUBLISH_TIMELOCK_SECS` defaults to 2 days and `TESTNET_DEPLOY=true` is the mandatory loud opt-in for any non-default delay. Publish is TWO phases (`…Propose`, then `…Execute` at/after the printed ETAs), reading the registry address from `PROTOCOL_REGISTRY`. `ProtocolVersions.sol` authors the single owner-hidden `dogtag-levelb/1` version as THREE values - `levelBContracts()`, `levelBArtifacts()`, and the binding - so the FIRST publication uses the two-axis scheme from the start (there is nothing to migrate). **`dogtag-levelb/1` IS PUBLISHED and active on both axes with its binding** (executed 2026-08-03): `getDiscoverySet(keccak256("dogtag-levelb/1"))` returns a nine-word record, `getActiveArtifactSet` resolves, `minAppVersion` is `1.4.0`, and the ledger's `_publication` note says the same. Note the registry was REDEPLOYED first so it could carry `PUBLISH_TIMELOCK` 0 and publish with no wait (`_protocol_registry_redeploy`); the earlier instance is not a write target. The getter is `getDiscoverySet` - a call to a selector the contract lacks reverts with EMPTY returndata, and reading that as "nothing is published" would be a dispatcher refusal mistaken for an answer, while an UNKNOWN key reverts with the contract's own named reason `"unknown discovery set"`. Publishing remains a deliberate captain-authorized step rather than part of the deploy. See `docs/PROTOCOL_REGISTRY_RUNBOOK.md`.
...
VR=$(python3 -c "import json;print(json.load(open('contracts/deployments/roax.json'))['VerificationRegistryConsent'])")
cast call "$VR" "zkVerifier()(address)" --rpc-url https://devrpc.roax.net
```

The bundled zkey's sha256 must be the ceremony output paired with whatever `zkVerifier()` returns.
That is `Groth16VerifierConsent` in `roax.json`, paired with the frozen consent zkey sha256 `f83a111f…`.
The launch verifier was redeployed from unmodified source and its runtime is byte-identical to the previously deployed frozen-ceremony verifier, so the on-chain VK did not move and the bundled pair is unchanged - see `_frozen_verifier`.
Do not transcribe the address into new places; `roax.json` owns it.
...
- **`authorizeClone` is deliberately NOT composed**, against the handoff note in
  `docs/ISSUER_OWNERSHIP.md` §3 that names S-9 as an intended consumer. It requires
...
Same shape as `ProviderRegistry.isWhitelistedFor`'s `msg.sender` branch, which is the reason
`ISSUER_REGISTRY_ADDR` cannot move; it just fails in the opposite direction here.
...
`0x6240dded`, called on the per-recordType issuer clone via `eth_call` against
`https://devrpc.roax.net`. Useful to verify a revoke rather than trusting the API's echo.
**Clones exist to call now** - the 2026-08-04 walk left a VACCINATION and a TRAVEL_CLEARANCE clone on
the deployed core (see the ledger's `_provisioning`), so there IS a per-recordType clone address to substitute; the earlier government clones belong to a superseded
factory and `rootIssuer` resolves nothing for them here.
...
This is the same `msg.sender` shape recorded above for `isWhitelistedFor`, failing
...
then go nowhere. `attachService`, `setServiceStanding`, the issuance grant, `setVerifierCapability`
and `setResolverApproved` close it (the issuance grant was `setIssuanceCapability` when this section
was written; #143 re-keyed it onto the address as `setRights(account, RIGHT_ISSUE)`, and the admin
backend sends that today - read `stacks/admin/api/src/routes.rs`, not this sentence's history) (`stacks/admin/api/src/routes.rs`,
...
registrar reaches the moment it has finished attach -> `setServiceStanding(ACTIVE)` -> the issuance
grant (now `setRights`, see above) is four terms held and this one false, which is exactly what the live ROAX
...
`tests/provider_journey.rs` on **`setVerifierCapability`**, not on the issuance grant: the latter
404s on a service that was never attached, and under propose-only nothing executes, so the

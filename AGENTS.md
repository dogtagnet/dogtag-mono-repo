...
- **THE FINDING - a genuine credential whose signer was delisted AFTER issuance is refused as a forgery.** `DogTagIssuer.sol:82` states the rule in the contract's own words ("delisting is forward-only") and `adminRevoke` exists as the retroactive lever precisely because `delistFor` is not one. The write path guarantees the historical fact independently: `issue()` is `onlyWhitelisted`, sets `issuedBy[r] = msg.sender` in the same call, and `rootIssuer[r]` is write-once and writable only from inside a clone's `issue()` - so for any root resolvable through the factory, "this signer was authorised at issuance" is true BY CONSTRUCTION. The mandatory pillar nevertheless reads `isWhitelistedFor`, a CURRENT-state getter, so an ordinary key rotation, a retirement or a lapsed licence renders every credential that signer ever issued as a forgery, fleet-wide. **A current-state whitelist read cannot produce a false PASS on a factory-resolved root** (the write path forbids it), so its only effect on that axis is false NEGATIVES.
  **Deliberately NOT fixed here.** That verdict formula is `integrity && onchain && issuerWhitelisted === true` in five surfaces the repo requires to agree (`packages/ui/src/wallet/verifyCredential.ts`, government-api `verify`, vet-api `verify_credential`, `crates/dogtag-standard-rs/src/verify.rs`, both mobile importers); moving it in one is a product decision and would split them. `signerDelistedAfterIssuance.expected` therefore pins what the code ACTUALLY does, and `divergence` is the report. A test asserts the divergence field is non-empty for exactly this scenario, so deleting it to make the catalogue read cleanly fails.
...
- **Verified by MUTATION, not by reading the diff.** Filtering the grant history to at-or-before the issuance point, the registry comparison, and feeding the unanchored-forgery scenario an unreachable chain instead of an answering one were each broken in turn; each reddened its own named test (including `distinguishes the two delisting cases - the check is not simply reading the current state` and `was refused by a factory that WAS reachable and answering`). A catalogue whose assertions cannot fail certifies itself.

...
Three things about that guard are load-bearing and each is easy to undo:
...
Answering the historical question for generation 2 needs an `IssuanceCapabilitySet` decoder on all five surfaces, which is NOT built — `ProviderRegistry` has no deployed address, so it could be validated against nothing. Until then a generation-2 root is honestly *unresolvable*, never refused.

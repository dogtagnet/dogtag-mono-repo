...
- **Ingest loop / reorg / resume (`src/indexer.rs`).** Only blocks below `head - CONFIRMATIONS` are indexed (finality buffer). The resume cursor (highest indexed block + its hash) is persisted after every chunk. Each tick re-checks the cursor block's stored hash against the live chain; on a mismatch it `delete_from_block(rewind)` + rewinds the cursor + rebuilds the known-clone set from the store, then re-scans (idempotent). `rebuild_known_clones()` on startup re-derives the clone set from previously-indexed `IssuerCreated` rows.
...
- **Demo mode** (`INDEXER_DEMO_MODE=1`): scripted `MemLogSource` history (deploy → whitelist → issue×2 → verify → revoke on the gov clone, plus a demo-groomer issuance on the DOG_PROFILE clone) + `MemStore`, `CONFIRMATIONS=0`, and two well-known tokens — `dogtag-indexer-oversight-demo-token` (unscoped) and `dogtag-indexer-vet-demo-token` (scoped to the DOG_PROFILE clone + demo-groomer signer). Lets the unscoped/scoped feeds return real decoded data with no node/Mongo.
- **Tests:** unit (`scope`/`store` modules) + `tests/query_api.rs` drives the real ingest loop + HTTP router end-to-end (unscoped vs scoped counts, scope-cannot-be-widened, filters/stats/issuers, 401 auth, idempotent re-scan, reorg rollback of orphaned events). All hermetic (MemLogSource + MemStore), in `cargo test -p indexer-api`.

...
- **Verified by MUTATION, not by reading the diff.** Four mutations, each reddening its own named tests: unfiltering the grant history from the issuance point, dropping the registry comparison, feeding the unanchored-forgery scenario an unreachable chain instead of an answering one, and editing the delisted-after expectation down to match the bug (six red). Named catchers include `distinguishes the two delisting cases - the check is not simply reading the current state`, `was refused by a factory that WAS reachable and answering`, and `MUST still verify - the scenario's expectation says so, and is not edited to match the code`. A catalogue whose assertions cannot fail certifies itself.

...
- **Implemented in four places**: `packages/ui/src/wallet/verifyCredential.ts` (`verdict = integrity && onchain && issuerWhitelisted === true`), `stacks/government/api/src/routes.rs::verify` and `stacks/vet/api/src/routes.rs::verify_credential` (both `issuer_whitelisted == Some(true)`), and the mobile importers (`RoaxRpc.issuerWhitelistPillar` + `RecordImporter.foldIssuerWhitelist`, a MONOTONE fold applied *after* the issuance mapping so it can only tighten a verdict). An operator-supplied signer is only an optional *expected*-signer assertion that tightens the pillar further; it can never enable it.
- **The three agree on the PILLAR's verdict, and diverge only in how an unanswerable read surfaces.** Do not "fix" the divergences below by aligning them blindly — each follows the host surface's own error convention:
  - a **blank `recordType`**: mobile guards it and returns `Unknown` (→ `UNVERIFIED`); web and government-api hash the empty string, which no clone's `recordType()` can equal, so the mismatch rule makes it a definite `false`.
  - an **unreadable chain read** (RPC error / revert): mobile → `Unknown` → a storable `UNVERIFIED`; web → the promise REJECTS (a toast, no result — deliberately stricter than mobile, so a broken read is never silently a pass); government-api → HTTP 502 with no verification row written.
...
- **`FACTORY_ADDR` is a SECURITY setting on vet-api/groomer-api, and its absence fails QUIETLY.** Unlike admin-api, where a missing factory makes routes answer a loud `FACTORY_ADDR not configured`, an unconfigured factory here just makes the pillar report `unavailableNoFactoryConfigured` and leaves the verdict alone — a forged `issuer.documentStore` is then refused by nothing but integrity. That fail-open is deliberate (our own misconfiguration is not evidence about a credential), which is exactly why the wiring matters: `stacks/vet/.env.example` and all three `demo-up.sh` vet-api instances (vet, groomer, prover — all serve this route) pass it. A MALFORMED value is treated differently from an absent one and returns HTTP 500 rather than degrading to fail-open, since a deployment that set the value intended to check.
- **Known remaining gap — the SDK path, NOT this endpoint**: `crates/dogtag-standard-rs/src/verify.rs::verify` (reached from vet-api via `crate::verify::third_party_verify`, i.e. `POST /records/import`) still makes every read against `doc.issuer.document_store` and compares `issued_by` to `doc.protocol.issuer_signer` — a SECOND attacker-controlled field, since `protocol` is also outside the Merkle root — with `Err(_) => FragmentState::Valid` fail-open. It has no factory anchor at all.
- **`MemChain` can emulate the attack, and a test that could not fail is worse than no test.** Its `issue()` mirrors `registerRoot` (write-once root→clone index; a second clone claiming a root errors `root taken`) and it stamps `issued_by = self.signer`, so the honest path can only ever produce answers a genuine clone would give. To pose the forgery use `MemChain::with_hostile_clone(addr, is_valid, issued_at, issued_by, record_type)` — a non-factory contract whose reads return attacker-chosen values while `root_issuer` deliberately does not name it.

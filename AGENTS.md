...
- **The binding's published-ness is checked at EXECUTE, not propose.** That is both stricter (a set deprecated during the window cannot slip through) and what lets the FIRST rollout run all three timelocks CONCURRENTLY - propose the sets and the binding together, wait once, then execute sets-then-binding. So publishing is still a two-phase script.
- `deprecateContractSet`/`deprecateArtifactSet` flip `active=false` immediately (a safety lever), NEVER delete - history stays pinned so old records self-route (§7.3). Each axis is an independent lever: retiring a compromised artifact set does not touch the trio, and the app still stops (the two `active` bits are AND-ed at the server seam).
...
That operational lever is now **WIRED, not merely documented**: `active` is a real member of BOTH `dogtag_prover::manifest::OnchainContractSet` and `OnchainArtifactSet` (mirroring the two Solidity `active` bits) and rides through `reconcile` into `Reconciliation::contract_set`/`::artifact_set`, from which `anchor_from_reconciliation` SOURCES it - AND-ing the two, so deprecating EITHER axis fails closed - so a chain-deprecated version yields `TrustedAnchor { active: false }` and the validator fails closed with `DeprecatedVersion`.
...
P4 still does NOT add a live Rust `ProtocolRegistry` eth-call reader (P3 deferred that) - the app reads `getContractSet`/`getActiveArtifactSet` natively into `TrustedAnchor.active` - but the bit now flows end-to-end through the reconcile/mapping path, so the defense is enforceable from both the app path and the server path.

...
- **Implemented in three places**: `packages/ui/src/wallet/verifyCredential.ts` (`verdict = integrity && onchain && issuerWhitelisted === true`), `stacks/government/api/src/routes.rs::verify` (`issuer_whitelisted == Some(true)`), and the mobile importers (`RoaxRpc.issuerWhitelistPillar` + `RecordImporter.foldIssuerWhitelist`, a MONOTONE fold applied *after* the issuance mapping so it can only tighten a verdict). An operator-supplied signer is only an optional *expected*-signer assertion that tightens the pillar further; it can never enable it.

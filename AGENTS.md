...
### The issuer-whitelist pillar is MANDATORY, and resolves its own signer

**A `null` pillar must never contribute a pass.** `check_integrity` folds only `data` + `privacy.obfuscated`, so the entire top-level `issuer` block — `name`, `domain`, and critically `documentStore`, the address every `isValid()` is made *against* — is OUTSIDE the Merkle root and attacker-controlled. Point `documentStore` at a contract you control that returns `true` from `isValid`, and integrity AND the on-chain read both pass. The whitelist pillar is the only check that catches it, so it cannot be optional.

- **Resolve the signer from the chain, never from an operator or the document.** `DogTagIssuer.issuedBy(root)` is the H-1 originator (`issuedBy[r] = msg.sender`, set under `onlyWhitelisted`), so a genuinely issued root's signer was whitelisted for its record type at issuance by construction. Then `IssuerRegistry.isWhitelistedFor(keccak256(recordType), thatSigner)` — against **the verifier's OWN configured registry** (`ISSUER_REGISTRY_ADDR` / bundled `roax.json` / `DEPLOYED_ADDRESSES`). If the registry ever came from the document the attacker would supply both sides of the question and the pillar would be theatre.
- **Tri-state, and only a definite `true` passes.** `true` = whitelisted; `false` = resolved but not authorized (a real authenticity failure); `null` = unresolvable, e.g. `issuedBy` returned the zero address — **indeterminate, and an unanswered check is not a passed check**. Treat zero-address as `null` *before* calling the registry; asking whether `0x0` is whitelisted yields a definite `false` for the wrong reason.
- **Live on ROAX**: `issuedBy(0x1736…ae5d)` → `0xc6c0904f…e39D`, `isWhitelistedFor(keccak("TRAVEL_CLEARANCE"), …)` → `true`, and `clone.recordType()` equals `cast keccak "TRAVEL_CLEARANCE"`. Genuine credentials keep passing; verify this before tightening the pillar further.
- **Implemented in three places, which must stay in agreement**: `packages/ui/src/wallet/verifyCredential.ts` (`verdict = integrity && onchain && issuerWhitelisted === true`), `stacks/government/api/src/routes.rs::verify` (`issuer_whitelisted == Some(true)`), and the mobile importers (`RecordImporter.foldIssuerWhitelist`, a MONOTONE fold applied *after* the issuance mapping so it can only tighten a verdict). An operator-supplied signer is now only an optional *expected*-signer assertion that tightens the pillar further; it can never enable it.
- **What this pillar does NOT catch**: relabelling `issuer.name`/`issuer.domain` alone. It authenticates the issuing KEY, not the label rendered beside it, so a relabelled credential with an intact `documentStore` still verifies — pinned deliberately by `a_name_only_relabel_is_out_of_this_pillars_reach`. Binding `issuer.domain` to the root-covered `data.issuer` DID is the separate issuer-identity assertion shipping with the DNS issuer-binding work. **Known remaining gap**: `stacks/vet/api`'s own `verify_credential` relay still treats the pillar as optional; the browser panel no longer uses it, but a direct API caller does.

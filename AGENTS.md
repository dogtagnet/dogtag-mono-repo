...
- **Provider directory (`GET /v1/businesses`, `POST /v1/businesses/nearest`) — interim admin source, on-chain source after S-10.** `Directory` retains the last complete, successfully decoded public admin `/v1/businesses` response, including nullable `geo` and all five optional BUSINESS contacts (`phone`, `whatsapp`, `telegram`, `email`, `website`), and never exposes `hmacSecret`. Static signer-name seeds and issuer applications are NOT provider rows because they cannot supply those fields. Never-loaded/unconfigured is 503; a successfully read empty source is a 200 page with `businesses:[]`. `GET` pages source-order results. Optional `name` (trimmed case/diacritic-insensitive substring), repeatable `kind` (trimmed case-insensitive exact; repeated values OR, e.g. `kind=vet&kind=groomer`), and `limit`/`offset` filter/page it; `type` is the same repeatable existing-client compatibility alias, but mixing the spellings is rejected. Kinds are opaque caller-selected values, not a server allowlist: current rows may be vet/groomer/admin/government, and future values still filter normally. `geo:null` remains in GET name/kind pages. Every response carries `total`, `limit`, `offset`, and `hasMore`.
  ...
  - **Nearest is server-side, body-only, coarse, and paged.** `POST /v1/businesses/nearest` takes `{"approximateLat":number,"approximateLng":number}` only after the device rounds the current fix to at most three decimals (~100 m). Query parameters remain name/repeatable kind-or-type/limit/offset; there is NO radius. Located matches are ordered by robust Haversine distance with source-order ties, page-selected without sorting the entire match set, and returned with `distanceKm`; contact-only `geo:null` rows cannot enter a nearest page. The position is never in a URL, response, cache key, trace/metric label, log, Mongo row, or audit record, and the response is `Cache-Control: private, no-store`.
  - **The app disclosure is part of the contract.** Immediately beside the action that starts location permission, Android and iOS say: “Your approximate location is sent to DogTag to find nearby vets and groomers. It is not stored.” Coarsening happens before the JSON body is constructed; the server's three-decimal rejection is only defense-in-depth. Nearest requests always carry the owner-owned `kind=vet&kind=groomer` restriction, consume the returned distance/order, and page rather than recomputing across a full list.
...
## Current provider search: coarse body-only position, server-side nearest, paged list

The captain superseded the full-fetch/on-device-distance model after sizing the production directory at
hundreds of thousands of providers. Native Nearby now uses the indexer's
`POST /v1/businesses/nearest`: the app requests coarse when-in-use location only after a tap, rounds both
coordinates to three decimals **before any network request**, and sends the approximation in a JSON body.
The service computes and returns `distanceKm`; each request selects its `limit`/`offset` window from
one immutable in-memory snapshot, and the phone does not rescan or remeasure the directory.

- The body is exactly `{"approximateLat":number,"approximateLng":number}`. Position never belongs in
  the URL, query, response, cache key, trace span, metric label, log, Mongo collection, or audit row.
  `Cache-Control: private, no-store` is mandatory on the response. Server rejection of values finer
  than three decimals is defense-in-depth; the privacy gain exists only because the client rounds first.
...
- The disclosure beside the permission action is explicit: “Your approximate location is sent to
  DogTag to find nearby vets and groomers. It is not stored.” Do not restore any “never leaves this
  phone” copy.
...
- Native personalized pages are not cached or persisted. Each page validates `total`/`limit`/`offset`,

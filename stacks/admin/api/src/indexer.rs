//! Oversight-indexer client - the admin/central stack's **UNSCOPED** consumer of the PR-4 indexer
//! (plan PR-B, Part 4 option A: a standalone indexer service the admin backend queries unscoped).
//!
//! The indexer (`stacks/indexer/api`) scans the ROAX factory / registry / DogTagIssuer-clone /
//! VerificationRegistry logs into a non-PII, scope-enforced event index and serves a bearer-gated
//! query API on `:46001`. The token the admin backend presents resolves **server-side** to
//! `Scope::Unscoped` (government/oversight): every event across every issuer, no client-supplied
//! filter can widen it. The admin backend is that indexer's cross-issuer consumer; it does NO
//! `eth_getLogs` of its own (confirmed: the admin `chain.rs` only does point reads + writes).
//!
//! This mirrors `business.rs`'s outbound-HTTP pattern: a trait (`OversightFeed`) so handlers stay
//! testable, a real `reqwest` impl (`HttpOversightFeed`), a `DisabledFeed` for when the indexer is
//! unconfigured (every call → `NotConfigured`, surfaced as 503), and a `MemFeed` for hermetic tests.
//!
//! Doctrine: the feed is non-PII (roots are salted Poseidon commitments, `dogTagId` a non-personal
//! SBT id, every address a public on-chain signer). The admin backend re-enriches it with its own
//! authoritative signer→business directory (`crate::directory`) and serves it unscoped.

use async_trait::async_trait;
use serde_json::Value;

/// The unscoped feed filters passed through to the indexer's `/v1/events` query params. Every field
...
impl std::fmt::Display for FeedError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            FeedError::NotConfigured => write!(f, "oversight indexer not configured (set INDEXER_API_BASE)"),
...
/// The oversight-indexer read surface the admin control plane consumes (unscoped).
#[async_trait]
pub trait OversightFeed: Send + Sync {
    ...
    /// `GET /v1/issuers` - per-clone issued/revoked/active counts.
    async fn issuers(&self) -> Result<Value, FeedError>;
    /// `GET /v1/status` - indexer progress + finality watermark (chain-health input for PR-D).
...
/// Real `reqwest` client for the oversight indexer, presenting the **unscoped** bearer token.
...
/// `NotConfigured` (handlers translate that to 503 "indexer not configured"). The rest of the admin
/// backend keeps working; only the activity/counts surfaces are unavailable.
...
/// `/v1/status` payloads shaped exactly like the real indexer's responses, so route wiring +
/// directory re-enrichment are exercised without a live indexer. Mirrors `MemChain`/`MockBusinessClient`.
...
mod tests {
    ...
    fn feed_query_emits_only_set_pairs_and_skips_blanks() {
        ...
        assert!(!pairs.iter().any(|(k, _)| *k == "signer"), "blank signer skipped");
        assert!(!pairs.iter().any(|(k, _)| *k == "offset"), "unset offset absent");
    }
    ...
    async fn mem_feed_returns_canned_payloads() {
        let f = MemFeed::new().with_stats(serde_json::json!({ "activeCredentials": 3 }));
        assert!(f.configured());
        assert_eq!(f.stats().await.unwrap()["activeCredentials"], 3);
    }
}

...
- `verify_sessions` - verifier-side audit rows of recorded `Verified` events `{sessionId, relayer, purpose, recordType, challenge, status, nullifier, txHash, disclosedKeyPaths[], appointmentId?/clientId?/petId?, ts}` - **owner-blind by construction**: the row has no subject field and the consent proof has no signal that could fill one. The optional appointment/client/pet ids are the shop's OWN business context (below), set when the operator started the verification from a booking; they are never exposed by the unauthenticated `GET /x/{token}` resolver.
- `crm_clients`, `crm_appointments`, `crm_verifications` — the **shop's own** customer book: clients (owner contact particulars + their embedded pets, each optionally carrying a `dogTagId`), its bookings `{clientId, petId, service, startAt/endAt (unix seconds), status, groomer, notes}`, and the central searchable history of verifications it performed, joined to the appointment + client where there is one. A verification row keeps only the public on-chain facts plus the disclosed **keyPaths** — never their values, and never the owner's `subject` wallet, which would create a client→wallet linkage the protocol withholds from a verifier.
- `clients`, `pets_cache` — imported pet profiles/owners (the `/import/pull` cache; distinct from `crm_clients` above).
- `appointments` — **replica** of the central-owned booking `{appointmentId, rev, state, slot, gcalEventId}` (§9.1 is the source of truth); distinct from the shop's own `crm_appointments`.

...
| `FACTORY_ADDR` | **all** | DogTagIssuerFactory. On admin: the `createIssuer`/`predictIssuer` target + the Ownable owner whose key gates deploys. On **vet and groomer it is a SECURITY setting**: its write-once `rootIssuer[R]` is what tells `POST /verify/credential` which clone really issued a credential instead of believing the document's own `issuer.documentStore` (outside the Merkle root, so attacker-chosen). It covers BOTH verifying routes: `POST /verify/credential` and `POST /import/pull` (the SDK-backed third-party verify). Unset → that pillar reports `unavailableNoFactoryConfigured` on both and a forged document is refused by nothing but integrity. MALFORMED → the route answers **500** rather than degrading to that fail-open state | (roax.json, pre-filled) | per chain; set it on **every** verifying deployment, the groomer included |
...
| `INDEXER_GENERATIONS` | indexer only | atomic JSON array of watched contract generations: each object carries `factory`, `issuerRegistry`, owner-hidden `verificationRegistry`, and its `seedClones`. The normalized factory is the immutable `generation` stamped on every event. Empty/malformed/ambiguous sets refuse startup; unknown emitters remain anti-spoof-dropped. `/v1/status.watchedGenerations` reports the exact active set, so an omitted generation is visible even when the feed is empty. Configure a new object **before** it emits; adding one after the finalized cursor passed its events requires a fresh rebuild/rescan from `START_BLOCK` at/before the oldest generation | current generation pre-filled in `stacks/indexer/.env.example` | every generation the indexer must observe; do not combine with deprecated singleton watch variables |

...
>
> **Track 1 - signer-1 Phase-2 (EXECUTED, historical record).**
> On 2026-07-05 (block 123835) an interim hand-off executed on-chain: the three governance authorities (`DogTagIssuerFactory` `Ownable2Step` ownership, the `IssuerRegistry` `WHITELIST_ADMIN` role, and the `IssuerRegistry`/`VerificationRegistry`/`DogTagSBT` `DEFAULT_ADMIN_ROLE` - the latter two being the since-retired owner-revealing registry and SBT) moved off the deployer EOA (`0x119F8c7F…`) onto the single governance signer **signer-1 `0x8E27E117…`** (recorded in `deployments/roax.json` `_governance`; see AGENTS.md "Governance / admin").
> The deployer EOA is therefore **not role-free**: it still holds the retired owner-revealing SBT's `ISSUER_ROLE` and its known record-type whitelists, so never treat it as a neutral/spent key, and retiring those legacy issuance capabilities is separate, unfinished work.
>
> **Track 2 - full `N`-of-`M` multisig (PENDING, and the code has since been DELETED).**
...
> It previously said "code shipped", and that is no longer true: `script/GovernanceMigration.sol`, `script/MigrateGovernance.s.sol` and `test/GovernanceMigration.t.sol` were all removed when the contract set collapsed to one launch generation.
> So every `forge script` invocation below names a file that does not exist, and this document is now a RECORD OF THE INTENDED PROCEDURE rather than a runnable one - reimplementing it against the launch set's own governance surface is the first step of actually doing it, not a detail.
...
That key can whitelist issuers, mass-revoke roots, and (after the 2-day timelock) swap the verifier, while the legacy deployer EOA still retains the retired owner-revealing SBT's `ISSUER_ROLE`/whitelists to mint on that retired contract.
...
| a legacy plain-`AccessControlEnumerable` SBT instance (historical case; not a current target) | plain `AccessControlEnumerable` | atomic `grantRole`→`revokeRole` (see note) |
...
> The retired owner-revealing contracts (`VerificationRegistry`, `DogTagSBT`, `ConsentKeyRegistry`, the non-consent `Groth16Verifier`) are no longer governance targets; `MigrateGovernance.s.sol` reads the owner-hidden `VerificationRegistryConsent`/`DogTagSBTConsent` targets from `deployments/roax.json`.
> Their already-deployed instances remain on-chain as history, and the Track-1 hand-off (above) that moved their admin to signer-1 is a historical record.
...
The retired owner-revealing SBT (`0x1FB8…`) is such a plain-`AccessControlEnumerable` instance - a historical case only, since it is no longer a migration target.

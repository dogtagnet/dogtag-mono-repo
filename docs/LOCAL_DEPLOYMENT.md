...
| `FACTORY_ADDR` | **REQUIRED** - the `DogTagIssuerFactory`. Beyond the admin console's `createIssuer`/`predictIssuer`, its write-once `rootIssuer[R]` is what lets **every vet-api instance** (vet, groomer AND prover - all three serve `POST /verify/credential`) resolve which clone really issued a credential instead of believing the document's own `issuer.documentStore`. Unset on a real deployment ⇒ that pillar reports `unavailableNoFactoryConfigured` and a forged document is refused by nothing but integrity | none - `demo-up.sh` resolves it from env, then `DOGTAG_ISSUER_FACTORY_ADDR`, then `contracts/deployments/roax.json`, and fails fast if all three are empty |

...
| `POST /v1/records/:root/share` | custodian 🔒 | mint a **one-time record-share QR token** (32 hex, 180s) → `{qrUrl: <DEPLOYMENT_URL>/r/<token>, root, ttlSecs, expiresAt}` |
| `GET /v1/verifications` | audit | the verification audit log |
| `POST /verify/session/start` | **verifier** 🔒 | start an owner-hidden verify session → `{qrUrl: <DEPLOYMENT_URL>/x/<token>?a=<relayer>, sessionId, ttlSecs, expiresAt}`. Refuses BEFORE showing a QR: 503 without a signer, **403 `relayer not whitelisted for this purpose`**, 503 without a verification registry |
| `GET /verify/history` | audit 🔒 | owner-hidden verification sessions (purpose/relayer/nullifier/tx — never owner identity) |
...
| `GET /r/:id` | **public** | **overloaded by id shape** — a 32-hex segment is a one-time record-share token, resolved to the wrapped doc as JSON and CONSUMED; anything else is a receipt id → the server-rendered, PII-free HTML status page (status-only, no Section A/B/C content), whose provenance row states outright when the backend is simulated |
| `GET /x/:token` | **public** | resolve a verify-session export token → `{sessionId, relayer, purpose, recordType, challenge, unverifiedClaims}` (non-consuming) |
| `POST /v1/verify/consent` | **verifier** | submit the owner-hidden consent proof (authed by the session's export token, or an operator bearer for a cold submit) |
| `GET /verify/session/:id` | **verifier** | poll a verify session — operator bearer **or** `?token=<exportToken>` |

**Phone handoff (owner ⇄ authority).** The last four rows plus `GET /r/:token` are the surface the owner's
phone drives, and they are **byte-identical to vet-api's** — same paths, same token shapes (32 hex), same
TTLs (share 180s, export 600s), same consumption rules (share consumed on first read; export peeked, with
...
gated by the `VERIFY:<purpose>` whitelist namespace, which is **separate from the issuer roles this authority
already holds**. Being a government buys no shortcut and no weaker privacy model: nothing on this path — on
chain or in the government's own DB — reveals who the owner is or what the credential says. The
paste-a-wrappedDoc `POST /v1/verify` stays as the no-owner-present fallback for checking a document.
...
1. **`DogTagIssuer` clones for the government record types.** ❌ **NOT DONE on the launch set.**
   No clone exists for either record type. Verified on chain 2026-08-03: `providerCount()` on the deployed `ProviderRegistry` is **0**, so no provider is registered, nothing has been created from the factory, and nothing has been issued.
   Earlier government clones did exist on a superseded set; they are not clones of this factory, so `DogTagIssuerFactory.rootIssuer` resolves nothing for them here and they are not a target to reuse.
   Leave both `TRAVEL_CLEARANCE_ISSUER_ADDR` and `EU_HEALTH_CERT_ISSUER_ADDR` unset until a clone is genuinely deployed, so `/issue` reports the issuer as `null` and dry-runs rather than anchoring somewhere wrong.
   **The clone must be bound to the authority the rest of the stack reads** - that failure mode is unchanged in kind and is the reason a preflight exists: a clone bound to a core nobody uses fails closed on the first issuance and passes every other check until then.
2. **Government signer onboarding.** ❌ **NOT DONE on the launch set.**
   `whitelistFor` no longer exists anywhere; authority is service-scoped, and onboarding is a registrar SEQUENCE on `ProviderRegistry` - `registerProvider` (which writes standing `PENDING`, so this alone leaves the provider inert) → `setProviderStanding(ACTIVE)` → `setServiceCreationApproval`, then `attachService`, `confirmServiceOwner` and `setIssuanceCapability(service, signer, true)` once the clone exists.
   `scripts/demo-provision-government.sh` still drives the OLD `whitelistFor` shape and has not been updated for this, so do not read it as a working path against the launch set.

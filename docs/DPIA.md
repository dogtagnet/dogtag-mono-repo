...
1. **The issuing vet holds owner identity by design.**
   ...
   **The identity is now COMMITTED into `R` as hidden, selectively-disclosable Merkle leaves (D1, implemented).**
   One `owner.identity.*` attribute leaf per non-blank field (`fullName`, `country`, `docNumber`), salted with a fresh vet-generated 16-byte salt, folded into `R` **on the owner's device** (the wallet seed never leaves the phone) and verified into `R` by the vet at custodial-bind before anything is anchored: the attestation-integrity gate is a full-leaf-list commitment check, requiring the device to open every attribute leaf and to match the identity openings EXACTLY against the vet's own stored set, so a device can neither replace a vet-attested value nor inject an extra identity leaf the vet never saw.
   ...
   - **Who holds the openings off-chain:** the issuing vet and the owner's device. The vet's session row keeps the identity `{keyPath, salt, value}` triples, and at custodial-bind the vet additionally receives the opening of every attribute leaf - the device-random pet-attribute salts included - as the full-leaf-list commitment the integrity gate rebuilds `R` from. That reveal is deliberate and zero-cost: the vet supplied every attribute value in the first place, so the openings add nothing it did not already know. The owner's device persists the same triples as disclosure openings, inside the mobile backup contract of `docs/MOBILE_OWNER_SECRET.md`.

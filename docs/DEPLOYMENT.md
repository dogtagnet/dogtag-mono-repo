...
| IssuerDomainRegistry (on-chain half of the issuer↔domain binding) | `0xD3B121FEaCde93b95288912EAdbB10824550FdBF` | Deployed but **EMPTY**: `boundCloneCount == 0`, so this does NOT make domain verification work - it only makes a claim readable once one is published; until then every clone's on-chain domain check reports itself unavailable. Additive (reads the factory + `IssuerRegistry`, writes to neither); deploy with `DeployIssuerDomainRegistry.s.sol`, never `Deploy.s.sol`. The getter is `issuerRegistry()`, not `registry()`. Consumers are fallback-free (`ISSUER_DOMAIN_REGISTRY_ADDR` + both compile-time mobile bundles), so an unset value fails silently. The admin portal's `VITE_ISSUER_DOMAIN_REGISTRY_ADDR` and the bench's two issuer-domain rows were REMOVED - with `boundCloneCount == 0` they could never answer. See [ISSUER_DOMAIN_BINDING.md](./ISSUER_DOMAIN_BINDING.md) |

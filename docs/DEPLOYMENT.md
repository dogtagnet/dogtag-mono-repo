...
| VerificationRegistryConsent (Level-B, owner-blind) | `0x53F988Ae0124b96069d90CBC78E6245FeB01E125` | Deployed M4 (block 195443) + validated, **not yet live**: the apps still send Level-A proofs and Level-B custodial tags need M5, so the cutover is **M7**. Additive - it does NOT replace the Level-A registry. **This instance is provisional:** its `sbt` is immutable and bound to the Level-A `DogTagSBT`, so the M5 deploy (not yet broadcast) will redeploy this same code against `DogTagSBTConsent` and supersede this address (safe - never live, zero `Verified` events). 4-arg `recordVerificationZK(a,b,c,pub[7])`, `pub = [dogTagId, purpose, relayer, nullifier, R, recordType, deadline]`, owner-blind `Verified` (no `subject`). Deployed runtime is byte-identical to source (6317 bytes). Details + the exhaustive cutover list: `contracts/deployments/roax.json` `_m4_consent_registry` / `_m5_custodial_issuance`; AGENTS.md "Level-B `VerificationRegistryConsent` (M4)" + "M5 as-built". `0x57A2998…` = `_preErasureGate_legacy` (RETIRED, never live: predates the `ownerOf` erasure gate) |
...
| Groth16VerifierConsent | `0x272be146C0aEd6401000E9Aa8241201F6f0fdF1a` | Level-B consent verifier - wired into `VerificationRegistryConsent` (M4) |

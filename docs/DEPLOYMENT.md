...
| VerificationRegistryConsent (Level-B, owner-blind) | `0x53F988Ae0124b96069d90CBC78E6245FeB01E125` | Deployed M4 (block 195443) + validated, **not yet live**: the apps still send Level-A proofs and Level-B custodial tags need M5, so the cutover is **M7**. Additive - it does NOT replace the Level-A registry. 4-arg `recordVerificationZK(a,b,c,pub[7])`, `pub = [dogTagId, purpose, relayer, nullifier, R, recordType, deadline]`, owner-blind `Verified` (no `subject`). **⚠ STALE DEPLOY:** this instance predates the `ownerOf` token-existence gate, so it lacks the burn/erasure gate and its runtime no longer matches source - it MUST be redeployed before the M7 cutover. Details + the exhaustive cutover list: `contracts/deployments/roax.json` `_m4_consent_registry`; AGENTS.md "Level-B `VerificationRegistryConsent` (M4)". Deployed runtime is byte-identical to source (6317 bytes). `0x57A2998…` = `_preErasureGate_legacy` (RETIRED, never live: predates the `ownerOf` erasure gate) |

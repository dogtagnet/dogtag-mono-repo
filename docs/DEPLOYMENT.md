...
| **VerificationRegistry** (Level-A; **the live one**) | **`0x4E2f0996e1CB4E24F1053346f3da2186906835E8`** | `0x8bA836eCe9a27c43049aCcC26eB5a1579c1FcFA1` = `_4arg_legacy`; `0x19C1B5f80c41EE864149500bdF998Dd18aec2a43` = `_preMetaTx_legacy`; `0xb4FbbDb50D86c5208D9278413ca05c5eE309b1e8` = `_zk0_legacy` |
| VerificationRegistryConsent (Level-B, owner-blind; canonical M5 target) | `0xb9B313C17fD8725Bb50A7f41121ac4Cf5F4fec87` | Deployed M5 (block 202602) + bytecode/wiring verified against `DogTagSBTConsent` `0x96Cba458…`; **not live** and no *shipped* consumer changed. M-3 added an additive, off-by-default relay route to vet-api (`POST /verify/consent/levelb`, enabled only by `VERIFICATION_REGISTRY_CONSENT_ADDR`; no shipped device posts to it), so the Level-A registry remains live until the **M7** cutover. `0x53F988Ae…` = `_M4_mutableRoot_legacy` (**DEPRECATED / DO NOT USE for Level-B**; immutable `sbt` points at the mutable Level-A SBT; never live; zero `Verified`); `0x57A2998…` = `_preErasureGate_legacy` (RETIRED; lacks the erasure gate). 4-arg `recordVerificationZK(a,b,c,pub[7])`, `pub = [dogTagId, purpose, relayer, nullifier, R, recordType, deadline]`, owner-blind `Verified` (no `subject`). Details: `contracts/deployments/roax.json` `_m5_custodial_issuance`; the exhaustive file-by-file M7 cutover list stays at `_m4_consent_registry.m7_cutover` (that key is deprecated as an *address*, but still owns the checklist); AGENTS.md "M5 as-built" |
...
| DogTagSBT (Level-A; live) | `0x1FB8986573Ac36d532cF7d5a5352202B094D4233` | Mutable-root Level-A profile/credential mint target; **do not use for new Level-B credentials** |
| DogTagSBTConsent (Level-B; canonical M5 target) | `0x96Cba4580D79bc9b8e51Fc1B3a044A29592AfFFc` | Deployed block 202601; write-once `profileRoot`; neutral custodian `0x637A5146…`; verified. vet-api's additive `POST /profiles/issue/custodial-bind` (M-2) can mint through it, but is **off unless `SBT_CONSENT_ADDR` + `PROFILE_ISSUER_ADDR` are set** (else 503) and no shipped device posts to it, so there is **no live traffic until M7** |
...
| Groth16VerifierConsent | `0x272be146C0aEd6401000E9Aa8241201F6f0fdF1a` | Level-B consent verifier - wired into canonical M5 `VerificationRegistryConsent` |
...
| deployer EOA (genesis) | `0x119F8c7F6D7EC10E7376983739C6f46cF9CC3E96` | Governance/admin authority removed in Phase-2, but legacy Level-A `ISSUER_ROLE` + record-type whitelists remain; **not a neutral custodian** |
...
| **clone / documentStore** | A per-recordType issuer contract (cloned from `DogTagIssuerImpl`) that anchors documents for a record type (e.g. VACCINATION). Backend env `*_ISSUER_ADDR` points at it. **Trap:** `PROFILE_DOCUMENT_STORE` is the exception — it defaults to the **SBT** address, because under Level-A the SBT doubles as the `DOG_PROFILE` document store and `issue()` is never called on it. So the Level-B `PROFILE_ISSUER_ADDR` (which *is* `issue(R)`'d) must be a **real factory-deployed clone**, never the SBT and never a copy of `PROFILE_DOCUMENT_STORE` — `issue(R)` sent to the SBT reverts. |

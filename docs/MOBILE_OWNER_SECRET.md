...
one difference in provenance: their salts are generated by the ISSUING VET (which verifies each
leaf's inclusion in `R` at custodial-bind), not by the device.

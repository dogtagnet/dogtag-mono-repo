...
**D1: the identity leaves join this retained set.**
The vet-attested `owner.identity.*` attributes (`fullName`, `country`, `docNumber`) are ordinary
attribute leaves in the same `attributes` list - same schema, same file, same retention rules - with
one difference in provenance: their salts are generated by the ISSUING VET (which, at
custodial-bind, requires the posted identity openings to exactly match its own stored set while
rebuilding `R` from the full leaf list), not by the device.
Their `{keyPath, salt, value}` openings are what later power owner-picked selective disclosure
(`ProfileDisclosure`), so losing them costs the tag its disclosure capability exactly as losing a
pet-attribute opening costs the tree its rebuildability.
The issuing vet retains the same triples on its session row, which is the re-obtain path.

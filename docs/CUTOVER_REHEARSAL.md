...
- **C-6, C-10b, C-11 and C-12 take one transaction per relayer or signer**, and those sets are the same KYC reconciliation. A list with invented addresses would read as ready, so the mechanics are exercised on the fork but no such list is published. `docs/CUTOVER_TRANSACTIONS.md` names every excluded step, so the rendered list cannot be mistaken for the whole cutover.

...
- **C-7 (`ServiceDomainResolver`, `ProviderDirectory`) is not in the list.** They have slack and gate nothing else - the plan's own reason - and they were not deployed at the time of this rehearsal. They have since been deployed live on ROAX (2026-08-01, `_c7_typed_resolvers` in `contracts/deployments/roax.json`) and are DEPLOYED AND UNWIRED: approved by no registrar, selected by no provider, read by no consumer. Their absence from the rehearsed list is unchanged - the rehearsal is a record of what was walked, not of what is deployed now.

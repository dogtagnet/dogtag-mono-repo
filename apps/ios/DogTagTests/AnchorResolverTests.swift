...
final class AnchorResolverTests: XCTestCase {
    ...
    func testARecordOfAnyOtherWidthFailsClosedRatherThanDecodingAtTheWrongIndices() {
        let eightWords = String(discoverySetGolden.prefix(64 * 8))
        let tenWords = discoverySetGolden + word("01")
        XCTAssertNil(
            AnchorResolver.decodeDiscoverySet(eightWords),
            "an 8-word return must not decode as the 9-word discovery set")
        XCTAssertNil(
            AnchorResolver.decodeDiscoverySet(tenWords),
            "a 10-word return must not decode as the 9-word discovery set")
        // ...and the 9-word record itself does decode, so neither refusal is vacuous.
        XCTAssertNotNil(AnchorResolver.decodeDiscoverySet(discoverySetGolden))
    }
    ...
    func testTheTwoAxesAreKeyedSeparately() {
    ...
    // MARK: - GOLDEN vectors from the real ABI encoder (independent of this decoder's author)
    ...
    /// which asserts these same bytes from the contract end. That shared literal is what makes the
    /// three files one contract rather than three independent guesses - and they had already drifted
    /// apart once, when the Solidity end was made synthetic and this pair was left holding a live
    /// chain capture, so the comment claiming they were pinned together was false for a while.
    ///
    /// THE ADDRESSES ARE SYNTHETIC, DELIBERATELY. What this vector pins is the record's ARITY and
    /// MEMBER ORDER, which is independent of which addresses the members hold; using the live
    /// deployed ones bought nothing and put real addresses in the tree, which is what `make
    /// check-addresses` exists to refuse. They are distinct per member so a member-order regression
    /// cannot pass by two slots holding the same value.
    ///
    ...
    private let discoverySetGolden =
    ...
    /// The golden record, decoded. Each member is asserted at its OWN value, which is what makes this
    /// a member-order check: the five addresses are distinct, so reading any one slot out of place
    /// fails here rather than passing on a coincidence. `discoverySetId` is
    /// `keccak256("dogtag-levelb/1")` and `circuitId` is `keccak256("consent.circom/DogTagConsent(6)")`,
    /// so both are checkable against the constants above rather than only against themselves.
    func testDecodeDiscoverySetGolden() {
        let d = AnchorResolver.decodeDiscoverySet(discoverySetGolden)
        XCTAssertNotNil(d)
        XCTAssertEqual(d?.discoverySetId, "0x36a8d69d16a9f540fa11be5f0311ebd5efd8e971b66cd704a6e197ee15b01b3d")
        XCTAssertEqual(d?.factory, "0x1c9ac2eb3f1a2d4b5c6d7e8f90a1b2c3d4e5f607")
        XCTAssertEqual(d?.verificationRegistry, "0x2b4d6f8a0c1e3a5b7d9f0e2c4a6b8d0f1e3a5c70")
    ...
    func testTheRootIndexIsTheFactory() {
        let d = AnchorResolver.decodeDiscoverySet(discoverySetGolden)
        XCTAssertNotNil(d)
        XCTAssertEqual(d?.rootIndex, d?.factory)
        XCTAssertEqual(d?.rootIndex, "0x1c9ac2eb3f1a2d4b5c6d7e8f90a1b2c3d4e5f607")
    }

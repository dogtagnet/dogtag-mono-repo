...
final class AnchorResolverTests: XCTestCase {
    ...
    /// The EXACT bytes `getDiscoverySet(keccak256("dogtag-levelb/1"))` returns from the published
    /// record on ROAX, captured with `cast call`. Nine words, one per member of the static tuple:
    ...
    /// ABI-model error. Recapture it if the record's shape or member order ever moves. The Kotlin
    /// mirror carries the identical bytes.
    ...
    /// `keccak256("consent.circom/DogTagConsent(6)")`, so both are checkable against the constants
    /// above rather than only against themselves.
    func testDecodeDiscoverySetGolden() {
        ...
        XCTAssertEqual(d?.factory, "0x5abdb6d61495f8b753069d582088d1f46283cc69")
        XCTAssertEqual(d?.verificationRegistry, "0x5cc19ab043be52becd4043aa66d40fb98ff66dff")
        XCTAssertEqual(d?.providerRegistry, "0x429c2ac735b1268fb0c65f7cf7746a1c0e8452aa")
    ...
    func testTheRootIndexIsTheFactory() {
        ...
        XCTAssertEqual(d?.rootIndex, "0x5abdb6d61495f8b753069d582088d1f46283cc69")
    }

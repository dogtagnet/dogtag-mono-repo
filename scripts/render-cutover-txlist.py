...
def main() -> int:
    ...
    if failed:
        out.append("**%d transaction(s) did NOT succeed.**\n" % len(failed))

    # Without this, a reader has no way to tell a complete list from one missing a whole C-step. The
    # renderer can refuse an UNMAPPED transaction it was given, but it cannot notice a step that was
    # never broadcast at all, so the exclusions are stated rather than inferred.
